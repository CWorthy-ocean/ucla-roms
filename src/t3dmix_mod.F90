module t3dmix_mod
#include "cppdefs.opt"
  implicit none
  private

#if defined SOLVE3D && ( defined TS_DIF2 || defined TS_DIF4 )
  public :: t3dmix_s, t3dmix_gp
contains

  subroutine t3dmix_s
    use param, only: ieast, iwest, jnorth, jsouth, nt, nsub_e, nsub_x
    use private_scratch, only: a2d
    implicit none
    integer(kind=4),save:: tile=0
    integer(kind=4) itrc

# include "compute_tile_bounds.h"
    do itrc=1,nt
      call t3dmix_s_tile (istr,iend,jstr,jend, itrc, A2d(1,1),&
      &A2d(1,2), A2d(1,3))
    enddo
  end subroutine t3dmix_s

  subroutine t3dmix_s_tile (istr,iend,jstr,jend, itrc, FX,FE,LapT)

! Computes horizontal mixing of tracers along S-surfaces.
    use grid, only: nz, umask, pmon_u, vmask, pnom_v, pn, pm
    use roms_mpi, only: exchange_xxx
    use dimensions, only: inode, jnode
    use param, only: ieast, iwest, jnorth, jsouth, np_xi, np_eta
    use mixing, only: diff2
    use ocean_vars, only: hz
    use scalars, only: dt, nnew, nrhs
    use tracers, only: t, wrt_t_dia
    use diagnostics, only:&
    &calc_diag, tmixx, tmixy, td,&
    &tdiag, diag_trc
    use roms_mpi, only: exchange_xxx

    implicit none
    integer(kind=4) istr,iend,jstr,jend, itrc, i,j,k
    real(kind=8), dimension(PRIVATE_2D_SCRATCH_ARRAY) :: FX,FE, LapT
# ifdef TS_DIF4
    integer(kind=4) imin,imax,jmin,jmax
# endif

# include "compute_auxiliary_bounds.h"

    do k=1,nz

# ifdef TS_DIF2

! Add in horizontal Laplacian diffusion along constant S-surfaces.
!--------------------------------------------------------------------
! Compute XI- and ETA-components of diffusive tracer flux.

      do j=jstr,jend
        do i=istr,iend+1
          FX(i,j)=0.25_8*(diff2(i,j,itrc)+diff2(i-1,j,itrc))&
          &*pmon_u(i,j)*(Hz(i,j,k)+Hz(i-1,j,k))*(&
          &t(i,j,k,nrhs,itrc)-t(i-1,j,k,nrhs,itrc)&
          &)&
#  ifdef MASKING
          &*umask(i,j)
#  endif
        enddo
      enddo
      do j=jstr,jend+1
        do i=istr,iend
          FE(i,j)=0.25_8*(diff2(i,j,itrc)+diff2(i,j-1,itrc))&
          &*pnom_v(i,j)*(Hz(i,j,k)+Hz(i,j-1,k))*(&
          &t(i,j,k,nrhs,itrc)-t(i,j-1,k,nrhs,itrc)&
          &)&
#  ifdef MASKING
          &*vmask(i,j)
#  endif
        enddo
      enddo
      if (diag_trc .and. wrt_t_dia(itrc) .and. calc_diag) then
        do j=jstr,jend
          do i=istr,iend+1
            Tdiag(i,j,k,tmixx,td(itrc)) =&
            &Tdiag(i,j,k,tmixx,td(itrc)) - FX(i,j)
          enddo
        enddo
        do j=jstr,jend+1
          do i=istr,iend
            Tdiag(i,j,k,tmixy,td(itrc)) =&
            &Tdiag(i,j,k,tmixy,td(itrc)) - FE(i,j)
          enddo
        enddo
      endif

! Add in horizontal diffusion of tracer [T m^3/s].

      do j=jstr,jend
        do i=istr,iend
          t(i,j,k,nnew,itrc)=t(i,j,k,nnew,itrc)+dt*pm(i,j)*pn(i,j)&
          &*(FX(i+1,j)-FX(i,j)+FE(i,j+1)-FE(i,j))&
          &/Hz(i,j,k)
        enddo
      enddo
# endif /* TS_DIF2 */

# ifdef TS_DIF4
#  ifdef EW_PERIODIC
      imin=istr-1
      imax=iend+1
#  else
      if (WESTERN_EDGE) then
        imin=istr
      else
        imin=istr-1
      endif
      if (EASTERN_EDGE) then
        imax=iend
      else
        imax=iend+1
      endif
#  endif
#  ifdef NS_PERIODIC
      jmin=jstr-1
      jmax=jend+1
#  else
      if (SOUTHERN_EDGE) then
        jmin=jstr
      else
        jmin=jstr-1
      endif
      if (NORTHERN_EDGE) then
        jmax=jend
      else
        jmax=jend+1
      endif
#  endif


!  Compute/Add in horizontal biharmonic diffusion along constant
!  S-surfaces.
!-----------------------------------------------------------------
!  The biharmonic operator is computed by applying the Laplacian
!  operator twice.
!
!  Compute horizontal tracer flux in the XI-direction at U-points.

      do j=jmin,jmax
        do i=imin,imax+1
          FX(i,j)=pmon_u(i,j)*0.5_8*(Hz(i,j,k)+Hz(i-1,j,k))*(&
          &t(i,j,k,nrhs,itrc)-t(i-1,j,k,nrhs,itrc)&
          &)&
#  ifdef MASKING
          &*umask(i,j)
#  endif
        enddo
      enddo

!  Compute horizontal tracer flux in the ETA-direction at V-points.

      do j=jmin,jmax+1
        do i=imin,imax
          FE(i,j)=pnom_v(i,j)*0.5_8*(Hz(i,j,k)+Hz(i,j-1,k))*(&
          &t(i,j,k,nrhs,itrc)-t(i,j-1,k,nrhs,itrc)&
          &)&
#  ifdef MASKING
          &*vmask(i,j)
#  endif
        enddo
      enddo

!  Compute the first Laplacian, without mixing coefficient.
!  Multiply by the metrics of the second Laplacian.
!  Save into work array "LapT".

      do j=jmin,jmax
        do i=imin,imax
          LapT(i,j)=( FX(i+1,j)-FX(i,j) +FE(i,j+1)-FE(i,j) )&
          &*pm(i,j)*pn(i,j)/Hz(i,j,k)
        enddo
      enddo

!  Apply boundary conditions (except periodic; closed or gradient)
!  to the first Laplacian.

#  ifndef EW_PERIODIC
      if (WESTERN_EDGE) then
        do j=jstr-1,jend+1
#   ifdef WESTERN_WALL
          LapT(istr-1,j)=0._8
#   else
          LapT(istr-1,j)=LapT(istr,j)
#   endif
        enddo
      endif
      if (EASTERN_EDGE) then
        do j=jstr-1,jend+1
#   ifdef EASTERN_WALL
          LapT(iend+1,j)=0._8
#   else
          LapT(iend+1,j)=LapT(iend,j)
#   endif
        enddo
      endif
# endif
# ifndef NS_PERIODIC
      if (SOUTHERN_EDGE) then
        do i=istr-1,iend+1
#  ifdef SOUTHERN_WALL
          LapT(i,jstr-1)=0._8
#  else
          LapT(i,jstr-1)=LapT(i,jstr)
#  endif
        enddo
      endif
      if (NORTHERN_EDGE) then
        do i=istr-1,iend+1
#  ifdef NORTHERN_WALL
          LapT(i,jend+1)=0._8
#  else
          LapT(i,jend+1)=LapT(i,jend)
#  endif
        enddo
      endif
# endif /* !NS_PERIODIC */

! Compute FX=d(LapT)/d(xi) and FE=d(LapT)/d(eta) terms.  After that
! compute and add in biharmonic mixing [T m^3/s]. Multiply by mixing
! coefficient.

      do j=jstr,jend
        do i=istr,iend+1
          FX(i,j)=0.25_8*(diff4(i,j,itrc)+diff4(i-1,j,itrc))&
          &*pmon_u(i,j)*(Hz(i,j,k)+Hz(i-1,j,k))&
          &*(LapT(i,j)-LapT(i-1,j))&
# ifdef MASKING
          &*umask(i,j)
# endif
        enddo
      enddo
      do j=jstr,jend+1
        do i=istr,iend
          FE(i,j)=0.25_8*(diff4(i,j,itrc)+diff4(i,j-1,itrc))&
          &*pnom_v(i,j)*(Hz(i,j,k)+Hz(i,j-1,k))&
          &*(LapT(i,j)-LapT(i,j-1))&
# ifdef MASKING
          &*vmask(i,j)
# endif
        enddo
      enddo
      do j=jstr,jend
        do i=istr,iend
          t(i,j,k,nnew,itrc)=t(i,j,k,nnew,itrc)+dt*pm(i,j)*pn(i,j)&
          &*(FX(i,j)-FX(i+1,j)+FE(i,j)-FE(i,j+1))&
          &/Hz(i,j,k)
        enddo
      enddo
#endif /* TS_DIF4 */
    enddo

# ifdef EXCHANGE
    call exchange_xxx(t(:,:,:,nnew,itrc) )
# endif
  end subroutine t3dmix_s_tile

  subroutine t3dmix_gp
    use param, only: ieast, iwest, jnorth, jsouth, nt, nsub_e, nsub_x
    use dimensions, only: inode, jnode
    use private_scratch, only: a2d, a3d
    implicit none
    integer(kind=4),save :: tile=0
    integer(kind=4) itrc

# include "compute_tile_bounds.h"
    do itrc=1,nt
      call t3dmix_gp_tile (istr,iend,jstr,jend, itrc, A3d(1,1),A2d(1,1),&
      &A2d(1,2), A2d(1,3),A2d(1,5),&
      &A2d(1,7), A2d(1,9), A2d(1,11), A2d(1,13))
    enddo
  end subroutine t3dmix_gp

  subroutine t3dmix_gp_tile (istr,iend,jstr,jend, itrc, LapT,FX,FE,&
  &FSC,dTdz, dTdx,dTde,dZdx,dZde)

    use param, only: ieast, iwest, jnorth, jsouth, np_xi, np_eta, nz
    use tracers, only: t, wrt_t_dia
    use dimensions, only: inode, jnode
    use diagnostics, only:&
    &calc_diag, tmixx, tmixy, td,&
    &tdiag, diag_trc
    use grid, only: nz, umask, pmon_u, vmask, pnom_v, pn, pm, dn_u, dm_v
    use mixing, only: diff2
    use ocean_vars, only: hz, z_r
    use scalars, only: dt, nnew, nrhs
    use roms_mpi, only: exchange_xxx
    implicit none
    integer(kind=4) istr,iend,jstr,jend, itrc, i,j,k,k1,k2
# ifdef TS_DIF4
    integer(kind=4) imin,imax,jmin,jmax
# endif
    real(kind=8), dimension(PRIVATE_2D_SCRATCH_ARRAY,0:nz) :: LapT
    real(kind=8), dimension(PRIVATE_2D_SCRATCH_ARRAY) :: FX,FE
    real(kind=8), dimension(PRIVATE_2D_SCRATCH_ARRAY,2) ::  FSC,dTdz,&
    &dTdx,dTde, dZdx,dZde
    real(kind=8) cff, cff1, cff2, cff3, cff4

# include "compute_auxiliary_bounds.h"

# ifdef MASKING
#  define SWITCH *
# else
#  define SWITCH !
# endif
# ifdef TS_DIF2

    !!! WARNING: RECURSIVE
! Compute horizontal (along geopotential     !!!   BLOCKING SEQUENCE
! surfaces) Laplacian diffusion terms.       !!!

    k2=1            ! vertical   dTdx,dTde(:,:,k2) k+1   rho-points
    do k=0,nz,+1     ! placement    dTdz,FSC(:,:,k2) k+1/2   W-points
      k1=k2         ! of scratch dTdx,dTde(:,:,k1) k     rho-points
      k2=3-k1       ! variables    dTdz,FSC(:,:,k1) k-1/2   W-points
      if (k<nz) then
        do j=jstr,jend
          do i=istr,iend+1
            cff=0.5_8*(pm(i,j)+pm(i-1,j)) SWITCH umask(i,j)
            dZdx(i,j,k2)=cff*(z_r(i,j,k+1) -z_r(i-1,j,k+1))
            dTdx(i,j,k2)=cff*(t(i,j,k+1,nrhs,itrc)&
            &-t(i-1,j,k+1,nrhs,itrc)&
            &)
          enddo
        enddo
        do j=jstr,jend+1
          do i=istr,iend
            cff=0.5_8*(pn(i,j)+pn(i,j-1)) SWITCH vmask(i,j)
            dZde(i,j,k2)=cff*(z_r(i,j,k+1) -z_r(i,j-1,k+1))
            dTde(i,j,k2)=cff*(t(i,j,k+1,nrhs,itrc)&
            &-t(i,j-1,k+1,nrhs,itrc)&
            &)
          enddo
        enddo
      endif

      if (k==0 .or. k==nz) then
        do j=jstr-1,jend+1
          do i=istr-1,iend+1
            dTdz(i,j,k2)=0.0_8     ! Set no-flux bottom or
            FSC(i,j,k2)=0.0_8       ! top boundary conditions.
          enddo
        enddo
      else
        do j=jstr-1,jend+1
          do i=istr-1,iend+1
            dTdz(i,j,k2)=(t(i,j,k+1,nrhs,itrc)-t(i,j,k,nrhs,itrc)&
            &)/(z_r(i,j,k+1)-z_r(i,j,k))
          enddo
        enddo
      endif

      if (k>0) then
        do j=jstr,jend
          do i=istr,iend+1
            FX(i,j)=0.25_8*(diff2(i,j,itrc)+diff2(i-1,j,itrc))&
            &*(Hz(i,j,k)+Hz(i-1,j,k))*dn_u(i,j)*(&
            &dTdx(i  ,j,k1)-0.5_8*(&
            &min(dZdx(i,j,k1),0._8)*(dTdz(i-1,j,k1)+dTdz(i,j,k2))&
            &+max(dZdx(i,j,k1),0._8)*(dTdz(i-1,j,k2)+dTdz(i,j,k1))&
            &))
          enddo
        enddo
        do j=jstr,jend+1
          do i=istr,iend
            FE(i,j)=0.25_8*(diff2(i,j,itrc)+diff2(i,j-1,itrc))&
            &*(Hz(i,j,k)+Hz(i,j-1,k))*dm_v(i,j)*(&
            &dTde(i,j,k1)-0.5_8*(&
            &min(dZde(i,j,k1),0._8)*(dTdz(i,j-1,k1)+dTdz(i,j,k2))&
            &+max(dZde(i,j,k1),0._8)*(dTdz(i,j-1,k2)+dTdz(i,j,k1))&
            &))
          enddo
        enddo
        if (k<nz) then
          do j=jstr,jend
            do i=istr,iend

              cff1=min(dZdx(i  ,j,k1),0._8)
              cff2=min(dZdx(i+1,j,k2),0._8)

              cff3=max(dZdx(i  ,j,k2),0._8)
              cff4=max(dZdx(i+1,j,k1),0._8)

              FSC(i,j,k2)=0.5_8*diff2(i,j,itrc)*(&
              &cff1*(cff1*dTdz(i,j,k2)-dTdx(i  ,j,k1))&
              &+cff2*(cff2*dTdz(i,j,k2)-dTdx(i+1,j,k2))&

              &+cff3*(cff3*dTdz(i,j,k2)-dTdx(i  ,j,k2))&
              &+cff4*(cff4*dTdz(i,j,k2)-dTdx(i+1,j,k1))&
              &)
              cff1=min(dZde(i,j  ,k1),0._8)
              cff2=min(dZde(i,j+1,k2),0._8)

              cff3=max(dZde(i,j  ,k2),0._8)
              cff4=max(dZde(i,j+1,k1),0._8)

              FSC(i,j,k2)=FSC(i,j,k2)+0.5_8*diff2(i,j,itrc)*(&
              &cff1*(cff1*dTdz(i,j,k2)-dTde(i,j  ,k1))&
              &+cff2*(cff2*dTdz(i,j,k2)-dTde(i,j+1,k2))&

              &+cff3*(cff3*dTdz(i,j,k2)-dTde(i,j  ,k2))&
              &+cff4*(cff4*dTdz(i,j,k2)-dTde(i,j+1,k1))&
              &)
            enddo
          enddo
        endif
        do j=jstr,jend
          do i=istr,iend
            t(i,j,k,nnew,itrc)=t(i,j,k,nnew,itrc)+dt*( pm(i,j)*&
            &pn(i,j)*( FX(i+1,j)-FX(i,j) +FE(i,j+1)-FE(i,j) )&
            &+FSC(i,j,k2)-FSC(i,j,k1)&
            &)/Hz(i,j,k)
          enddo
        enddo
      endif  ! <-- k>0
    enddo   ! --> k
# endif /* TS_DIF2 */
# ifdef TS_DIF4
#  ifndef EW_PERIODIC
    if (WESTERN_EDGE) then
      imin=istr
    else
      imin=istr-1
    endif
    if (EASTERN_EDGE) then
      imax=iend
    else
      imax=iend+1
    endif
#  else
    imin=istr-1
    imax=iend+1
#  endif
#  ifndef NS_PERIODIC
    if (SOUTHERN_EDGE) then
      jmin=jstr
    else
      jmin=jstr-1
    endif
    if (NORTHERN_EDGE) then
      jmax=jend
    else
      jmax=jend+1
    endif
#  else
    jmin=jstr-1
    jmax=jend+1
#  endif

! Compute horizontal biharmonic diffusion      !!! WARNING: RECURSIVE
! along geopotential surfaces. The biharmonic  !!!  BLOCKING SEQUENCE
! operator is computed by applying the rotated !!!
! Laplacian operator twice.

! THE FIRST LAPLACIAN: ! vertical   dTdx,dTde(:,:,k2)  k+1   rho-points
!                      ! placement    dTdz,FSC(:,:,k2) k+1/2   W-points
    k2=1             ! of scratch dTdx,dTde(:,:,k1)  k     rho-points
    do k=0,nz,+1      ! variables    dTdz,FSC(:,:,k1) k-1/2   W-points
      k1=k2
      k2=3-k1
      if (k<nz) then
        do j=jmin,jmax
          do i=imin,imax+1
            cff=0.5_8*(pm(i,j)+pm(i-1,j)) SWITCH umask(i,j)
            dZdx(i,j,k2)=cff*(z_r(i,j,k+1)-z_r(i-1,j,k+1))
            dTdx(i,j,k2)=cff*(t(i,j,k+1,nrhs,itrc)-&
            &t(i-1,j,k+1,nrhs,itrc))
          enddo
        enddo
        do j=jmin,jmax+1
          do i=imin,imax
            cff=0.5_8*(pn(i,j)+pn(i,j-1)) SWITCH vmask(i,j)
            dZde(i,j,k2)=cff*(z_r(i,j,k+1)-z_r(i,j-1,k+1))
            dTde(i,j,k2)=cff*(t(i,j,k+1,nrhs,itrc)-&
            &t(i,j-1,k+1,nrhs,itrc))
          enddo
        enddo
      endif

      if (k==0 .or. k==nz) then
        do j=jmin-1,jmax+1
          do i=imin-1,imax+1
            dTdz(i,j,k2)=0.0_8
            FSC(i,j,k2)=0.0_8
          enddo
        enddo
      else
        do j=jmin-1,jmax+1
          do i=imin-1,imax+1
            dTdz(i,j,k2)=(t(i,j,k+1,nrhs,itrc)-t(i,j,k,nrhs,itrc))&
            &/(z_r(i,j,k+1)-z_r(i,j,k))
          enddo
        enddo
      endif

      if (k>0) then
        do j=jmin,jmax
          do i=imin,imax+1
            FX(i,j)=0.5_8*(Hz(i,j,k)+Hz(i-1,j,k))*dn_u(i,j)*(&
            &dTdx(i  ,j,k1)&
            &-0.5_8*( min(dZdx(i,j,k1),0._8)*(dTdz(i-1,j,k1)+dTdz(i,j,k2))&
            &+max(dZdx(i,j,k1),0._8)*(dTdz(i-1,j,k2)+dTdz(i,j,k1))&
            &))
          enddo
        enddo
        do j=jmin,jmax+1
          do i=imin,imax
            FE(i,j)=0.5_8*(Hz(i,j,k)+Hz(i,j-1,k))*dm_v(i,j)*(&
            &dTde(i,j,k1)&
            &-0.5_8*( min(dZde(i,j,k1),0._8)*(dTdz(i,j-1,k1)+dTdz(i,j,k2))&
            &+max(dZde(i,j,k1),0._8)*(dTdz(i,j-1,k2)+dTdz(i,j,k1))&
            &))
          enddo
        enddo
        if (k<nz) then
          do j=jmin,jmax
            do i=imin,imax
!**             cff1=min(dZdx(i,j,k1)+dZdx(i+1,j,k2),0._8)
!**             cff2=max(dZdx(i,j,k2)+dZdx(i+1,j,k1),0._8)
!**
!**             cff3=min(dZde(i,j,k1)+dZde(i,j+1,k2),0._8)
!**             cff4=max(dZde(i,j,k2)+dZde(i,j+1,k1),0._8)
!**
!**             FSC(i,j,k2)=0.25_8*( dTdz(i,j,k2)*( (cff1+cff2)**2
!**  &                                           +(cff3+cff4)**2)
!**
!**  &                           -cff1*(dTdx(i,j,k1)+dTdx(i+1,j,k2))
!**  &                           -cff2*(dTdx(i,j,k2)+dTdx(i+1,j,k1))
!**
!**  &                           -cff3*(dTde(i,j,k1)+dTde(i,j+1,k2))
!**  &                           -cff4*(dTde(i,j,k2)+dTde(i,j+1,k1))
!**  &                                                             )

              cff1=min(dZdx(i  ,j,k1),0._8)
              cff2=min(dZdx(i+1,j,k2),0._8)

              cff3=max(dZdx(i  ,j,k2),0._8)
              cff4=max(dZdx(i+1,j,k1),0._8)

              FSC(i,j,k2)=0.5_8*(&
              &cff1*(cff1*dTdz(i,j,k2)-dTdx(i  ,j,k1))&
              &+cff2*(cff2*dTdz(i,j,k2)-dTdx(i+1,j,k2))&

              &+cff3*(cff3*dTdz(i,j,k2)-dTdx(i  ,j,k2))&
              &+cff4*(cff4*dTdz(i,j,k2)-dTdx(i+1,j,k1))&
              &)
              cff1=min(dZde(i,j  ,k1),0._8)
              cff2=min(dZde(i,j+1,k2),0._8)

              cff3=max(dZde(i,j  ,k2),0._8)
              cff4=max(dZde(i,j+1,k1),0._8)

              FSC(i,j,k2)=FSC(i,j,k2)+0.5_8*(&
              &cff1*(cff1*dTdz(i,j,k2)-dTde(i,j  ,k1))&
              &+cff2*(cff2*dTdz(i,j,k2)-dTde(i,j+1,k2))&

              &+cff3*(cff3*dTdz(i,j,k2)-dTde(i,j  ,k2))&
              &+cff4*(cff4*dTdz(i,j,k2)-dTde(i,j+1,k1))&
              &)
            enddo
          enddo
        endif
        do j=jmin,jmax
          do i=imin,imax
            LapT(i,j,k)=( pm(i,j)*pn(i,j)*( FX(i+1,j)-FX(i,j)&
            &+FE(i,j+1)-FE(i,j) )&
            &+FSC(i,j,k2)-FSC(i,j,k1)&
            &)/Hz(i,j,k)
          enddo
        enddo
      endif   ! <-- k>0
    enddo   ! --> k

#  ifndef EW_PERIODIC
    if (WESTERN_EDGE) then            ! Apply lateral boundary
      do k=1,nz                        ! conditions to the first
        do j=jmin,jmax                ! Laplacian (in cases other
#   ifdef WESTERN_WALL
          LapT(istr-1,j,k)=0._8         ! than periodic: closed
#   else
          LapT(istr-1,j,k)=LapT(istr,j,k)      !  or gradient)
#   endif
        enddo
      enddo
    endif
    if (EASTERN_EDGE) then
      do k=1,nz
        do j=jmin,jmax
#   ifdef EASTERN_WALL
          LapT(iend+1,j,k)=0._8
#   else
          LapT(iend+1,j,k)=LapT(iend,j,k)
#   endif
        enddo
      enddo
    endif
#  endif /* !EW_PERIODIC */
#  ifndef NS_PERIODIC
    if (SOUTHERN_EDGE) then
      do k=1,nz
        do i=imin,imax
#   ifdef SOUTHERN_WALL
          LapT(i,jstr-1,k)=0._8
#   else
          LapT(i,jstr-1,k)=LapT(i,jstr,k)
#   endif
        enddo
      enddo
    endif
    if (NORTHERN_EDGE) then
      do k=1,nz
        do i=imin,imax
#   ifdef NORTHERN_WALL
          LapT(i,jend+1,k)=0._8
#   else
          LapT(i,jend+1,k)=LapT(i,jend,k)
#   endif
        enddo
      enddo
    endif
#  endif /* !NS_PERIODIC */
!
! THE SECOND LAPLACIAN
!
    k2=1
    do k=0,nz,+1
      k1=k2
      k2=3-k1

      if (k<nz) then
        do j=jstr,jend
          do i=istr,iend+1
            cff=0.5_8*(pm(i,j)+pm(i-1,j)) SWITCH umask(i,j)
            dZdx(i,j,k2)=cff*(z_r(i,j,k+1) -z_r(i-1,j,k+1))
            dTdx(i,j,k2)=cff*(LapT(i,j,k+1)-LapT(i-1,j,k+1))
          enddo
        enddo
        do j=jstr,jend+1
          do i=istr,iend
            cff=0.5_8*(pn(i,j)+pn(i,j-1)) SWITCH vmask(i,j)
            dZde(i,j,k2)=cff*(z_r(i,j,k+1) -z_r(i,j-1,k+1))
            dTde(i,j,k2)=cff*(LapT(i,j,k+1)-LapT(i,j-1,k+1))
          enddo
        enddo
      endif

      if (k==0 .or. k==nz) then
        do j=jstr-1,jend+1
          do i=istr-1,iend+1
            dTdz(i,j,k2)=0.0_8
            FSC(i,j,k2)=0.0_8
          enddo
        enddo
      else
        do j=jstr-1,jend+1
          do i=istr-1,iend+1
            dTdz(i,j,k2)=(LapT(i,j,k+1)-LapT(i,j,k))&
            &/(z_r(i,j,k+1)-z_r(i,j,k))
          enddo
        enddo
      endif

      if (k>0) then
        do j=jstr,jend
          do i=istr,iend+1
            FX(i,j)=-0.25_8*(diff4(i,j,itrc)+diff4(i-1,j,itrc))&
            &*(Hz(i,j,k)+Hz(i-1,j,k))*dn_u(i,j)*(&
            &dTdx(i  ,j,k1)-0.5_8*(&
            &min(dZdx(i,j,k1),0._8)*(dTdz(i-1,j,k1)+dTdz(i,j,k2))&
            &+max(dZdx(i,j,k1),0._8)*(dTdz(i-1,j,k2)+dTdz(i,j,k1))&
            &))
          enddo
        enddo
        do j=jstr,jend+1
          do i=istr,iend
            FE(i,j)=-0.25_8*(diff4(i,j,itrc)+diff4(i,j-1,itrc))&
            &*(Hz(i,j,k)+Hz(i,j-1,k))*dm_v(i,j)*(&
            &dTde(i,j,k1)-0.5_8*(&
            &min(dZde(i,j,k1),0._8)*(dTdz(i,j-1,k1)+dTdz(i,j,k2))&
            &+max(dZde(i,j,k1),0._8)*(dTdz(i,j-1,k2)+dTdz(i,j,k1))&
            &))
          enddo
        enddo
        if (k<nz) then
          do j=jstr,jend
            do i=istr,iend
!**             cff1=min(dZdx(i,j,k1)+dZdx(i+1,j,k2),0._8)
!**             cff2=max(dZdx(i,j,k2)+dZdx(i+1,j,k1),0._8)
!**
!**             cff3=min(dZde(i,j,k1)+dZde(i,j+1,k2),0._8)
!**             cff4=max(dZde(i,j,k2)+dZde(i,j+1,k1),0._8)
!**
!**             FSC(i,j,k2)=-0.25_8*diff4(i,j,itrc)*(
!**  &                             dTdz(i,j,k2)*( (cff1+cff2)**2
!**  &                                           +(cff3+cff4)**2)
!**
!**  &                           -cff1*(dTdx(i,j,k1)+dTdx(i+1,j,k2))
!**  &                           -cff2*(dTdx(i,j,k2)+dTdx(i+1,j,k1))
!**
!**  &                           -cff3*(dTde(i,j,k1)+dTde(i,j+1,k2))
!**  &                           -cff4*(dTde(i,j,k2)+dTde(i,j+1,k1))
!**  &                                                             )

              cff1=min(dZdx(i  ,j,k1),0._8)
              cff2=min(dZdx(i+1,j,k2),0._8)

              cff3=max(dZdx(i  ,j,k2),0._8)
              cff4=max(dZdx(i+1,j,k1),0._8)

              FSC(i,j,k2)=-0.5_8*diff4(i,j,itrc)*(&
              &cff1*(cff1*dTdz(i,j,k2)-dTdx(i  ,j,k1))&
              &+cff2*(cff2*dTdz(i,j,k2)-dTdx(i+1,j,k2))&

              &+cff3*(cff3*dTdz(i,j,k2)-dTdx(i  ,j,k2))&
              &+cff4*(cff4*dTdz(i,j,k2)-dTdx(i+1,j,k1))&
              &)
              cff1=min(dZde(i,j  ,k1),0._8)
              cff2=min(dZde(i,j+1,k2),0._8)

              cff3=max(dZde(i,j  ,k2),0._8)
              cff4=max(dZde(i,j+1,k1),0._8)

              FSC(i,j,k2)=FSC(i,j,k2)-0.5_8*diff4(i,j,itrc)*(&
              &cff1*(cff1*dTdz(i,j,k2)-dTde(i,j  ,k1))&
              &+cff2*(cff2*dTdz(i,j,k2)-dTde(i,j+1,k2))&

              &+cff3*(cff3*dTdz(i,j,k2)-dTde(i,j  ,k2))&
              &+cff4*(cff4*dTdz(i,j,k2)-dTde(i,j+1,k1))&
              &)
            enddo
          enddo
        endif
        do j=jstr,jend
          do i=istr,iend
            t(i,j,k,nnew,itrc)=t(i,j,k,nnew,itrc) + dt*( pm(i,j)&
            &*pn(i,j)*( FX(i+1,j)-FX(i,j) +FE(i,j+1)-FE(i,j) )&
            &+FSC(i,j,k2)-FSC(i,j,k1)    )/Hz(i,j,k)
          enddo
        enddo
      endif  ! <-- k>0
    enddo   ! --> k
# endif /* TS_DIF4 */

# ifdef EXCHANGE
    call exchange_xxx(t(:,:,:,nnew,itrc) )
# endif
  end subroutine t3dmix_gp_tile

#else
contains
  subroutine t3dmix_empty
  end subroutine t3dmix_empty
#endif
end module t3dmix_mod
