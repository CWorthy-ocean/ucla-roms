...
#include "cppdefs.opt"
  ...
  use param, only: mynode, nnodes, isw_corn, iwest, jsouth, jsw_corn, np_xi, np_eta
...
!---------------------------------------------------------
  subroutine def_vars_instant(ncid, varname, ndim, dimsize)  ![
    ...
    integer(kind=4),intent(in),dimension(:) :: dimsize

    ! local
    integer(kind=4) :: i, ierr, varid, did, xtype
    character(len=1) :: istr
    integer(kind=4),allocatable,dimension(:) :: dimid

    integer(kind=4),parameter :: default_prec = nf90_double
...
!----------------------------------------------------------------------
  subroutine wrt_instant_2D(var, varname) ![
...
!----------------------------------------------------------------------
  subroutine wrt_instant_3D(var, varname) ![
    implicit none
