module diag_mod
  ...
  character(len=8) :: module_name = "diag_mod"
  logical, public :: code_check_mode = .false.
  namelist /STDOUT_DIAG_SETTINGS/ code_check_mode
  public :: diag, read_nml_stdout_diag
...
#define SUM_BY_PAIRS
#define MAX_ADV_CFL
!--#define MAX_BY_VERT_CFL_ONLY

#define REPORT_MAX_UV
#define REPORT_POTEN_ENRG

  subroutine read_nml_stdout_diag
    use error_handling_mod, only: error_log
    use namelist_open_mod, only: check_nml_read
    use namelist_buffer_mod, only: namelist_lines
!     Read the "STDOUT_DIAG_SETTINGS" section of the namelist file
    integer(kind=4) ::  namelist_unit, ios
    character(len=20) :: sr_name = "read_nml_stdout_diag"
    character(len=512) :: msg = ""
    ! Read namelist

    read (namelist_lines, nml=STDOUT_DIAG_SETTINGS, iostat=ios, iomsg=msg)
    call check_nml_read(ios, 'STDOUT_DIAG_SETTINGS', module_name//'/'//sr_name, msg)

  end subroutine read_nml_stdout_diag
  ...
  subroutine diag
    ...
    use scalars, only: iic, ninfo

    implicit none
    character(len=4) :: sr_name = "diag"
...
#endif
  &)
    use comm_vars, only: area, first_time, tile_count, volume
    use diag_vars, only:&
    &avke, avke2b, avke3bc, avkesrf, avpe,&
    &avzeta, cu_adv, cu_w, i_cx_max, j_cx_max, k_cx_max, v2d_max
#ifdef SPLIT_EOS
    use eos_vars, only: qp1, rho1
#else
    use eos_vars, only: rho
#endif
    use grid, only: pn, pm, rmask
    use ocean_vars, only: u, hz, v, z_w, wi, we, flxv, flxu, z_r
    use param, only:&
    ...
    implicit none
    character(len=9) :: sr_name = "diag_tile"

    integer(kind=4) istr,iend,jstr,jend
    real(kind=8) :: walltime0 = 0._8
    real(kind=8), dimension(PRIVATE_2D_SCRATCH_ARRAY) :: dVol, ke, pe&
#ifdef SOLVE3D
    &, ke2b, ke3bc, kesrf, ub, vb
#endif
#ifdef MPI
...
# endif
    ...
    real(kind=QUAD) buff(15)
#endif

    real(kind=8) cff, dA, v2, my_v2d_max
    real(kind=QUAD) my_avzeta, my_ke,   my_pe&
#ifdef SOLVE3D
...
# else
    real(kind=8) my_v3d_max, my_v3bc_max
# endif
#endif
#ifdef SUM_BY_PAIRS
    integer(kind=4) isize,itg, js,jsize,jtg
#endif
    integer(kind=4), parameter :: max_check_line=512
...
# else
    my_v3d_max=0._8 ;  my_v3bc_max=0._8
# endif
    do j=jstr,jend+1
      do i=istr,iend+1
...
#  ifdef VERT_EXPL_ONLY
          cw=ciV*( max(We(i,j,k), 0._8) -min(We(i,j,k-1), 0._8) )
#  else
...
# endif
    ...
    do j=jstr,jend
      do i=istr,iend
        v2=0.5_8*( ubar(i,j,kstp)**2+ubar(i+1,j,kstp)**2&
...
# endif
        ...
        ke(i,j)=dA*0.5_8*(zeta(i,j,kstp)+h(i,j))*v2
      ...
      if (NNODES>1) then                   ! Perform global summation
        ...
        do while (size>1)
          ...
          if (mynode>=step .and. mynode<size) then
            buff(1)=avzeta
            buff(2)=avke                   ! This is essentially an
            buff(3)=avpe                   ! MPI_Reduce operation,
            buff(4)=avke2b                 ! although with a rather
            buff(5)=avke3bc                ! complicated function.
            buff(6)=avkesrf
            buff(7)=v2d_max
# ifdef SOLVE3D
#  ifdef MAX_ADV_CFL
...
!**     write(*,*) 'sending  ',mynode,'->',mynode-step,' size=',size
!**     write(*,*) 'sending  avke =', buff(3)/volume

            itag=mynode+400
            call MPI_Send (buff, 13, MPI_BUFF_TYPE, mynode-step,&
            &itag, ocean_grid_comm,          ierr)
          elseif (mynode < size-step) then
            itag=mynode+400+step
            call MPI_Recv (buff, 13, MPI_BUFF_TYPE, mynode+step,&
            &itag, ocean_grid_comm,  status, ierr)

!**     write(*,*) 'received ',mynode,'<-',mynode+step,' size=',size
!**     write(*,*) 'received avke =', buff(3)/volume
...
#   ifdef MAX_BY_VERT_CFL_ONLY
            if (buff(9) > Cu_W) then
#   else
            if (buff(8) > Cu_Adv) then
#   endif
              Cu_Adv=buff(8)
              Cu_W=buff(9)
              i_cx_max=buff(10) +0.0001_8
              j_cx_max=buff(11) +0.0001_8
              k_cx_max=buff(12) +0.0001_8
            endif
#  else
            v2=buff(8)
...
# if defined MAX_ADV_CFL
        ...
        !WALLTIME0 is MPI_Wtime() on node 0
        walltime0 = MPI_Wtime()-tstart
        if (first_time==0) then
          first_time=1
          if (code_check_mode) then
            write(*,3) 'STEP',  'KINETIC_ENRG',&                  ! Special for code checks
            &'BAROTR_KE', 'MAX_ADV_CFL', 'MAX_VERT_CFL'
          else
            write(*,1) 'STEP', 'time[DAYS]',&
            &'KINETIC_ENRG',& !  Normal log file output
            &'BAROTR_KE', 'MAX_ADV_CFL', 'MAX_VERT_CFL',&
            &'i_cx', 'j_cx', 'k_c', 'WALLTIME0'
        ...
        if (code_check_mode) then
          ...
          write(check_line,2,iostat=ierr)&
          &avke,  avke2b,  Cu_Adv,&
          &Cu_W, i_cx_max,  j_cx_max,  k_cx_max,&
          &walltime0
        endif
!$      $$  1   format(/,1X, A6, 2X, A10, 1X, A13, 1X, A13, 1X, A13, 1X, A13, 3X, A7, 1X, A7, 1X, A5, 1X, A12)
1       format(/1x,A,2x,A,1x,A,5x,A,8x,A,2(5x,A),3x,A,2(2x,A),10x,A)
2       format(ES18.11, ES17.10, 2F16.12, 2I7,I5, F12.3)
3       format(/2x,A,1x,A,5x,A,8x,A,2(5x,A),3x,A,2(1x,A),10x,A)
4       format(I2,1x,4ES23.16,F12.3)
...
! blowup) - raise a global error.
            ...
            elseif (check_line(i:i)<'0' .or. check_line(i:i)>'9') then
              ...
              &check_line(i:i) /= '.') then
                ...
                &info="Abnormal termination: BLOWUP",&
                &context=sr_name)
              endif
            endif
          enddo  ! <-- while(..)
