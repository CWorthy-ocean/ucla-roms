...
#include "cppdefs.opt"
  implicit none
  private
  public :: setup_grid1, setup_grid2
contains
  ...
  subroutine setup_grid1(tile)          ! grid: Compute various
    ...
    integer(kind=4) tile                          ! terms.
#include "compute_tile_bounds.h"
    call setup_Corls_tile(istr,iend,jstr,jend)
    call setup_grid1_tile(istr,iend,jstr,jend)
  end subroutine setup_grid1

  subroutine setup_Corls_tile(istr,iend,jstr,jend)
    use param, only: ieast, iwest, jnorth, jsouth
    use grid, only: f, latr, fomn, pn, pm
    use scalars, only: deg2rad, pi
...
! Coriolis frequency at North Pole is 4*pi/T where T is sidereal day
    ...
    cff1=4._8*pi/(23.9344699D0*3600.D0)
    do j=jstrR,jendR
      do i=istrR,iendR
...
# endif
    ...
    enddo
#endif
  end subroutine setup_Corls_tile
...
#endif
    dm_r, pm, dn_r, pn,&
    &pmon_u, dm_u, dn_u, pn_u,&
    &umask, rmask, pmask, vmask, ia_u, pnom_v, dn_p,&
    &pm_v, ia_v, dm_v, dn_v, dm_p
...
!  Compute m/n at horizontal U-points.
    ...
    do j=jstrR,jendR
      do i=istr,iendR
        ...
        dm_u(i,j)=2.0_8/(pm(i,j)+pm(i-1,j))
        dn_u(i,j)=2.0_8/(pn(i,j)+pn(i-1,j))
        pn_u(i,j)=0.5_8*(pn(i,j)+pn(i-1,j))
#ifdef MASKING
...
!  Compute n/m at horizontal V-points.
    ...
    do j=jstr,jendR
      do i=istrR,iendR
        ...
        dm_v(i,j)=2.0_8/(pm(i,j)+pm(i,j-1))
        dn_v(i,j)=2.0_8/(pn(i,j)+pn(i,j-1))
        pm_v(i,j)=0.5_8*(pm(i,j)+pm(i,j-1))
...
!     &                                      *rmask(i-1,j-1)
!***      if (gamma2 < 0._8) pmask(i,j)=2._8-pmask(i,j)

! Set no-slip boundary conditions on land-mask boundaries
! regardless of supplied value of gamma2.


        cff1=1._8  !<-- computation of off-diagonal nonlinear terms
        cff2=2._8

        if (rmask(i-1,j  )>0.5_8 .and. rmask(i,j  )>0.5_8 .and.&
        &rmask(i-1,j-1)>0.5_8 .and. rmask(i,j-1)>0.5_8) then
          pmask(i,j)=1._8

        elseif(rmask(i-1,j  )<0.5_8 .and. rmask(i,j  )>0.5_8 .and.&
        &rmask(i-1,j-1)>0.5_8 .and. rmask(i,j-1)>0.5_8) then
          pmask(i,j)=cff1
        elseif(rmask(i-1,j  )>0.5_8 .and. rmask(i,j  )<0.5_8 .and.&
        &rmask(i-1,j-1)>0.5_8 .and. rmask(i,j-1)>0.5_8) then
...
# endif
#endif /* EXCHANGE */
  end subroutine setup_grid1_tile
...
! boundaries.
    ...
    use param, only:&
    &ieast, iwest, jnorth, jsouth, mynode, nsub_e,&
    &nsub_x, ocean_grid_comm, np_xi, np_eta, nnodes
    use comm_vars, only:&
    &area, bc_crss, cg_max, cg_min, cu_cor,&
    &grdmax, grdmin, hmax, hmin, tile_count, volume
    use scalars, only: dt, dtfast, g
    use mpi_f08, only: mpi_real8, mpi_status_size
    use dimensions, only: inode, jnode
    use grid, only:&
    &rmask, h, pn, pm, f, umask, dn_u,&
    &vmask, dm_v
...
# endif
    ...
    real(kind=QUAD) buff(16)
#endif

#include "compute_auxiliary_bounds.h"

    my_hmin=+1.D+20              ! Here two-dimensional Courant
    my_hmax=-1.D+20              ! number is defined as
    my_grdmin=+1.D+20            !
    my_grdmax=-1.D+20            ! Cu = c*dt * sqrt(1/dx^2+1/dy^2)
    my_Cu_min=+1.D+20            !
    my_Cu_max=-1.D+20            ! where c=sqrt(g*h) is phase
    my_Cu_Cor=-1.D+20            ! speed for barotropic mode,
...
!$  OMP END CRITICAL (grd2_cr_rgn)

  end subroutine setup_grid2_tile

end module setup_grid_mod
