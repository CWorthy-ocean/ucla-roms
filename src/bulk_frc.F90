...
#include "cppdefs.opt"
  ...
  type (ncforce) :: nc_uwnd  = ncforce(&
  &vname='uwnd', tname='time' ) ! u-wind (input data in m/s at 10m)
  type (ncforce) :: nc_vwnd  = ncforce(&
  &vname='vwnd', tname='time' ) ! v-wind (input data in m/s at 10m)
  type (ncforce) :: nc_tair  = ncforce(&
  &vname='Tair', tname='time' ) ! Air temp (input data Degrees C at 2m)
  type (ncforce) :: nc_Q     = ncforce(&
  &vname='qair', tname='time' ) ! Q (Specific humidity (kg/kg)
  type (ncforce) :: nc_prate = ncforce(&
  &vname='rain', tname='time' ) ! Precipitation rate (cm/day)
  type (ncforce) :: nc_lwrad = ncforce(&
  &vname='lwrad',tname='time' ) ! Downward longwave radiation [W/m^2]
  type (ncforce) :: nc_swrad = ncforce(&
  &vname='swrad',tname='time' ) ! net shortwave radiation [W/m^2]

#if defined TAU_CORRECTION
  type (ncforce)  :: nc_taux = ncforce(vname='TauX_corr',tname='time'  )  ! The TAU_CORRECTION flag is used to activ
  type (ncforce)  :: nc_tauy = ncforce(vname='TauY_corr',tname='time'  )  ! bring bulk force field closer to that of
#endif

  character(len=8) :: module_name = "bulk_frc"
  real(kind=8),allocatable,public,dimension(:,:) ::  tair             ! air temp, used in step3d_t to add the heat o
  real(kind=8),allocatable,public,dimension(:,:) ::     Q             ! SPECIFIC humidity (kg/kg) (not relative), qa
  real(kind=8),allocatable,public,dimension(:,:) :: prate             ! prate (precipitation rate) - 'rain' variable
  real(kind=8),allocatable,public,dimension(:,:) :: evap              ! evap (evaporation rate)
  real(kind=8),allocatable,public,dimension(:,:) :: lwrad             ! Long wave radiation - get_bulk_rad.F & set_b
  !real:: srflx(GLOBAL_2D_ARRAY)                       ! short wave radiation: srflx stored in surf_flux.F module as

#if defined TAU_CORRECTION
  real(kind=8),allocatable,public,dimension(:,:) :: taux              ! The TAU_CORRECTION flag is used to active a
  real(kind=8),allocatable,public,dimension(:,:) :: tauy              ! bring bulk force field closer to that of the
#endif
...
!     Read the "SURF_FRC_SETTINGS" section of the namelist file
    ...
    ! Read namelist
    read (namelist_lines, nml=SURF_FRC_SETTINGS, iostat=ios, iomsg=msg)
    call check_nml_read(ios, 'SURF_FRC_SETTINGS', module_name//'/'//sr_name, msg)
  ...
  subroutine init_arrays_bulk_frc  ![
    implicit none
    allocate(  tair(GLOBAL_2D_ARRAY) ); tair = 0
    allocate(     Q(GLOBAL_2D_ARRAY) ); Q = 0
    allocate( prate(GLOBAL_2D_ARRAY) ); prate = 0
    allocate( evap(GLOBAL_2D_ARRAY) ); evap = 0
    allocate( lwrad(GLOBAL_2D_ARRAY) ); lwrad = 0

    allocate( wspd_used(GLOBAL_2D_ARRAY) ); wspd_used = 0
...
!      Response Experiment, JGR, 101, 3747-3764._8
...
!      103, 409-438._8
    ...
    integer(kind=4) i,j, IterMax,iter
    real(kind=8) a
    real(kind=8) rho0i
    real(kind=8) TseaC,TseaK,Qsea
    real(kind=8) TairC,TairK,rhoAir ! ,Qair
    real(kind=8) VisAir,Hlv ! Q, RH,
    real(kind=8) delW,delT,delQ
    real(kind=8) u10,Zo10,Cd10,Ch10,Ct10,Cd
    real(kind=8) Ct,CC,Ri,Ribcu,Zetu,L10
    real(kind=8) Wstar,Tstar,Qstar
    real(kind=8) ZoW,ZoT,ZoT10,ZoQ,ZoL,L,Rr,Bff
    real(kind=8) Wpsi,Tpsi,Qpsi
    real(kind=8) wspd0,Wgus,charn
...
!      real bulk_psiu,bulk_psit
    ...
    real(kind=8) hfsen,hflat,hflw,upvel


    real(kind=8) blk_Rgas,blk_ZW,blk_ZT,blk_ZQ,blk_beta,blk_Cpa,blk_Zabl
    parameter (blk_Rgas=287.1_8)
    parameter (blk_ZW=10.0_8)  ! The height at which the atm. Wind is taken
    parameter (blk_ZT=10.0_8)  ! The height at which the atm. Temp is taken
    parameter (blk_ZQ=10.0_8)  ! The height at which the atm. Humi.is taken
    parameter (blk_Zabl=600.0_8)
    parameter (blk_beta=1.2_8)
    parameter (blk_Cpa=1004.67_8)
    real(kind=8) emiss_lw,SigmaSB
    parameter (emiss_lw=0.985_8)        ! longwave emissivity of ocean,
    ! ref. Dickey et al. (1994)
    parameter (SigmaSB=5.6697D-8)     ! Stefan-Boltzmann constant [m^2/K^4]
    real(kind=8) rhow,patm
    parameter (rhow=1000.0_8)
    parameter (patm=1010.0_8)
    real(kind=8) eps,r3
    parameter (eps=1.D-20)
    parameter (r3=1.0_8/3.0_8)
...
!!! CFB feedback
    ...
    real(kind=8) cfb_slope, cfb_offset
    parameter (cfb_slope=-0.0029_8)
    parameter (cfb_offset=0.008_8)
    real(kind=8)    Wspd_min, stau_ref

!!!      real aer, cer
    real(kind=8) aer(PRIVATE_2D_SCRATCH_ARRAY),&
    ...
    real(kind=8) wspd ! uwnd, vwnd, - DevinD commented
...
!!!     &      cpair=0.240_8*4186.8_8, ! specific heat for air in
!!!                                ! Joules/(kg*degC), where 4186.8 is
!!!                                ! to convert from cal/g to Joules/kg
    &cpi=1._8/Cp    ! inverse of specific heat for seawater
    ! [kg*degC/Joule], Cp is defined in "scalars"


    ! DevinD don't need these anymore!
    integer(kind=4) it1,it2, it3,it4, it5,it6, it7,it8, it9,it10
    real(kind=8) tmid, cff1,cff2, cff3,cff4, cff5,cff6, cff7,cff8,&
    &cff9,cff10, cff
# ifdef QCORRECTION
#  define SST_DATA
    integer(kind=4) it13,it14, it11,it12
    real(kind=8) cff13,cff14, cff11,cff12
# endif
# ifdef SFLX_CORR
#  define SSS_DATA
    integer(kind=4) it15,it16
    real(kind=8) cff15,cff16
# endif
# define WIND_DATA
# define TAIR_DATA
# define RHUM_DATA
...
!      real, parameter :: year2day=365.25_8,  day2year=1.D0/year2day
...
# endif
    ...
    enddo
!
!        if (cff1 < 0._8 .or. cff2 < 0._8) then
!          if (ZEROTH_TILE) then
...
!          RH=rhum(i,j)/100.0_8 ! To get specific humidity (if input data is pdamien's pacific model L217 COAREnew)
...
!
!          Esat(mb) = (1.0007_8+3.46D-6*patm)*6.1121_8*
!                  EXP(17.502_8*TairC(C)/(240.97_8+TairC(C)))
!
...
!
!          Q = 0.622 E/(P + (0.622_8-1)e)
!
!          Q(kg/kg) = 0.62197_8*(E(mb)/(patm(mb)-0.378_8*E(mb)))
!
!-----------------------------------------------------------------------
!
! air saturation vapor pressure (mb), using Teten formula.

!          cff=(1.0007_8+3.46D-6*patm)*6.1121_8
!     &        *exp(17.502_8*TairC/(240.97_8+TairC))

! specific humidity at Saturation, Qair (kg/kg).

!          Qair=0.62197_8*(cff/(patm-0.378_8*cff))
...
!
!          if (RH < 2._8) then                          !RH fraction
!            cff=cff*RH                                 !Vapor pres (mb)
!            Q=0.62197_8*(cff/(patm-0.378_8*cff))           !Spec hum (kg/kg)
!          else          !RH input was actually specific humidity in g/kg
!            Q=RH/1000.0_8                                !Spec Hum (kg/kg)
!          endif
...
! water saturation vapor pressure (mb), using Teten formula.
        ...
        cff=(1.0007_8+3.46D-6*patm)*6.1121_8*&
        &exp(17.502_8*TseaC/(240.97_8+TseaC))
...
! Qsea (kg/kg) from vapor pressure.
        ...
        Qsea=0.62197_8*(cff/(patm-0.378_8*cff))
!
!-----------------------------------------------------------------------
...
!
        ZoW=0.0001_8
        u10=delW*LOG(10.0_8/ZoW)/LOG(blk_ZW/ZoW)
        Wstar=0.035_8*u10
        Zo10=0.011_8*Wstar*Wstar/g+0.11_8*VisAir/Wstar
        Cd10=(vonKar/LOG(10.0_8/Zo10))**2
        Ch10=0.00115_8
        Ct10=Ch10/sqrt(Cd10)
        ZoT10=10.0_8/exp(vonKar/Ct10)
...
!
        ...
        CC=vonKar*Ct/Cd
        Ribcu=-blk_ZW/(blk_Zabl*0.004_8*blk_beta**3)
        Ri=-g*blk_ZW*(delT+0.61_8*TairK*delQ)/&
        &(TairK*delW*delW)
        if (Ri.lt.0.0_8) then
          Zetu=CC*Ri/(1.0_8+Ri/Ribcu)       ! Unstable
        else
          Zetu=CC*Ri/(1.0_8+3.0_8*Ri/CC)      ! Stable
        endif
        L10=blk_ZW/Zetu
        if (Zetu.gt.50.0_8) then
          IterMax=1
        else
          IterMax=3
        endif
...
!  1.0_8/(18.0_8-10.0_8).
!
        if (delW.gt.18.0_8) then
          charn=0.018_8
        elseif ((10.0_8.lt.delW).and.(delW.le.18.0_8)) then
          charn=0.011_8+0.125_8*(0.018_8-0.011_8)*(delW-10._8)
        else
          charn=0.011_8
        endif
!
...
!
        do Iter=1,IterMax
          ZoW=charn*Wstar*Wstar/g+0.11_8*VisAir/(Wstar+eps)
          Rr=ZoW*Wstar/VisAir
!
!  Compute Monin-Obukhov stability parameter, Z/L.
!
          ZoQ=MIN(1.15D-4,5.5D-5/Rr**0.6_8)
          ZoT=ZoQ
          ZoL=vonKar*g*blk_ZW*&
          &(Tstar*(1.0_8+0.61_8*Q(i,j))+0.61_8*TairK*Qstar)/&
          &(TairK*Wstar*Wstar*(1.0_8+0.61_8*Q(i,j))+eps)
          L=blk_ZW/(ZoL+eps)
!
...
!
          Bff=-g/TairK*Wstar*(Tstar+0.61_8*TairK*Qstar)
          if (Bff.gt.0.0_8) then
...
!
        upvel=-1.61_8*Wstar*Qstar-(1.0_8+1.61_8*Q(i,j))*Wstar*Tstar/TairK
...
!
        evap(i,j)=-cp*hflat/Hlv

        ! Divide prate by 100.*3600._8*24. to convert from [cm/day] to [m/s]
        swflx(i,j) = prate(i,j)*cmday2ms-evap(i,j)
        ! This is the old way of adding salt flux
!          stflx(i,j,isalt)=(evap(i,j)- ( prate(i,j)/(100._8*3600._8*24._8) ) )*t(i,j,N,nrhs,isalt)
#endif
# ifdef MASKING
...
!---------------------------------------------------------------
! Restrict stflx to prevent surface temperature to go below -1.8_8
! degrees C.
!---------------------------------------------------------------
...
!
    real(kind=8) r3
    parameter (r3=1.0_8/3.0_8)
    real(kind=8) Fw, cff, psic, psik, x, y
!
!-----------------------------------------------------------------------
...
!
    if (ZoL<0.0_8) then
      x=(1.0_8-15.0_8*ZoL)**0.25_8
      psik=2.0_8*LOG(0.5_8*(1.0_8+x))+LOG(0.5_8*(1.0_8+x*x))-&
      &2.0_8*ATAN(x)+0.5_8*pi
!
!  For very unstable conditions, use free-convection (Fairall).
!
      cff=SQRT(3.0_8)
      y=(1.0_8-10.15_8*ZoL)**r3
      psic=1.5_8*LOG(r3*(1.0_8+y+y*y))-&
      &cff*ATAN((1.0_8+2.0_8*y)/cff)+pi/cff
!
!  Match Kansas and free-convection forms with weighting Fw.
!
      cff=ZoL*ZoL
      Fw=cff/(1.0_8+cff)
      bulk_psiu=(1.0_8-Fw)*psik+Fw*psic
!
!  Stable conditions.
!
    else
      cff=MIN(50.0_8,0.35_8*ZoL)
      bulk_psiu=-((1.0_8+ZoL)+0.6667_8*(ZoL-14.28_8)/&
      &EXP(cff)+8.525_8)
    endif
...
!
    if (ZoL<0.0_8) then
      x=(1.0_8-15.0_8*ZoL)**0.5_8
      psik=2.0_8*LOG(0.5_8*(1.0_8+x))
!
!  For very unstable conditions, use free-convection (Fairall).
!
      cff=SQRT(3.0_8)
      y=(1.0_8-34.15_8*ZoL)**r3
      psic=1.5_8*LOG(r3*(1.0_8+y+y*y))-&
      &cff*ATAN((1.0_8+2.0_8*y)/cff)+pi/cff
!
!  Match Kansas and free-convection forms with weighting Fw.
!
      cff=ZoL*ZoL
      Fw=cff/(1.0_8+cff)
      bulk_psit=(1.0_8-Fw)*psik+Fw*psic
!
!  Stable conditions.
!
    else
      cff=MIN(50.0_8,0.35_8*ZoL)
      bulk_psit=-((1.0_8+2.0_8*ZoL)**1.5_8+&
      &0.6667_8*(ZoL-14.28_8)/EXP(cff)+8.525_8)
    endif
