...
# ifdef SPHERICAL
    call ncread(ncid,'lon_rho',lonr(x0:x1,y0:y1),fname=grdname)  ! Coordinates (lon,lat [degrees]) or (x,y [meters]) a
...
# ifdef CURVGRID
    ...
    call ncread(ncid,'angle',angler(x0:x1,y0:y1),fname=grdname)  ! Angle (radians) between XI-axis and EAST at RHO-poi
