...
#include "cppdefs.opt"
  ...
  use ocean_vars, only: z_w, u, v, hz, z_r, flxu, flxv
  use scalars, only: dt, nnew, iic, tdays, time
  use param, only:&
  &ocean_grid_comm, mynode, isw_corn, jsw_corn,&
  &obc_east, obc_west, obc_south, obc_north
  use grid, only: dn_u, dn_v, h, pm, pn, dm_v
  use dimensions, only: nx, ny, nz, inode, jnode, npx, npy
  use roms_read_write, only:&
  &dn_tm, create_file, store_string_att,&
  &particle_opt
  ...
  integer(kind=4) :: np=50                   ! local number of particles
  real(kind=8)    :: extra_space_fac = 1.5_8   ! buffer space to receive extra exchanged particles
  real(kind=8)    :: exchange_facx = 0.1_8     ! maximum number of particles for transfer in NS
  ...
  character(len=130) :: particle_string
  character(len=130) :: particle_num_string
  character(len=30) :: particle_conc_string
  character(len=30) :: buffer_string


  ! indices of various quantities in particle array
  integer(kind=4),parameter :: itag = 1   ! identification tag
  integer(kind=4),parameter :: ipx  = 2   ! x-position (index space)
  integer(kind=4),parameter :: ipy  = 3   ! y-position (index space)
  integer(kind=4),parameter :: ipz  = 4   ! z-position (index space)
  integer(kind=4),parameter :: ipu  = 5   ! x-velocity (m/s)
  integer(kind=4),parameter :: ipv  = 6   ! y-velocity (m/s)
  integer(kind=4),parameter :: ipw  = 7   ! z-velocity (m/s)
  integer(kind=4),parameter :: iprx = 8   ! dx-velocity (index space)
  integer(kind=4),parameter :: ipry = 9   ! dy-velocity (index space)
  integer(kind=4),parameter :: iprz = 10  ! dz-velocity (index space)
  integer(kind=4),parameter :: iprxm= 11  ! previous dx-velocity (index space)
  integer(kind=4),parameter :: iprym= 12  ! previous dy-velocity (index space)
  integer(kind=4),parameter :: iprzm= 13  ! previous dz-velocity (index space)
  integer(kind=4)           :: npv  = 13  ! total particle vars

  integer(kind=4)     :: ntag                  ! current max tag (lifetime particles)
  logical :: do_init = .true.
  logical :: part_first = .true.             ! First time step for particles

  real(kind=8),dimension(:,:),allocatable,target :: part    ! particles
  real(kind=8)   ,dimension(:),allocatable :: val ! needed for sorting
  integer(kind=4),dimension(:),allocatable :: idx ! needed for sorting
  real(kind=8),dimension(:),allocatable :: ptmp  ! needed for sorting
  real(kind=8),dimension(:,:,:),allocatable :: Wp    ! total vertical flux
  real(kind=8),dimension(:),allocatable :: zw
  real(kind=8),dimension(:) ,allocatable :: nbp_east,nbp_west ! for reseeding near open boundaries
  real(kind=8),dimension(:) ,allocatable :: nbp_north,nbp_south

  integer(kind=4) :: szx,szy,szc   !! size of mpi buffers for particle exchanges

  real(kind=8)                  :: output_time = 500 ! produces output at first timestep
  integer(kind=4)               :: total_rec = 0
  integer(kind=4) ::  npmx ! allocated space for particle array
  integer(kind=4) :: big_number= 10000000      ! added to particles tags
  ! to get unique numbers for all subdomains
  integer(kind=4) :: n_sur=0,n_bot=0

  interface interp
    module procedure  interp_2D, interp_3D
  end interface interp

  real(kind=8),allocatable,dimension(:) :: sn_nw, sn_n, sn_ne, rv_nw, rv_n, rv_ne,&
  &sn_w, sn_e, rv_w, rv_e, sn_sw, sn_s,&
  &sn_se, rv_sw, rv_s, rv_SE
  integer(kind=4) :: is_s, is_w, is_n, is_e
  integer(kind=4) :: is_sw,is_se,is_nw,is_ne
  integer(kind=4) :: rn_s, rn_w, rn_n, rn_e                       ! received msg buffer size
  integer(kind=4) :: rn_sw,rn_se,rn_nw,rn_ne

  public do_particles,init_particles,read_nml_particles

contains
!-------------------------------------------------
  subroutine read_nml_particles

!     Read the "PARTICLES_SETTINGS" section of the namelist file
...
!-------------------------------------------------
  subroutine init_particles![
    implicit none
    !local
    integer(kind=4) :: ip,i,j,lnp
    real(kind=8)    :: h_mn,dx_mn,dy_mn,est_np
    real(kind=8)    :: hvol
    ...
    if (full_seed) then ! seed for constant particle density
      ...
      dx_mn= 1._8/(sum(pm(1:nx,1:ny))/(nx*ny))
      dy_mn= 1._8/(sum(pn(1:nx,1:ny))/(nx*ny))
...
!-------------------------------------------------
  subroutine advance_particles![
    ...
    real(kind=8),dimension(:),pointer:: dpx,dpy,dpz
    real(kind=8),dimension(:),pointer:: dpxm,dpym,dpzm
    integer(kind=4) :: ip
...
!     endif
    ...
    px =  px + 1.5_8*dpx - 0.5_8*dpxm
    py =  py + 1.5_8*dpy - 0.5_8*dpym
    pz =  pz + 1.5_8*dpz - 0.5_8*dpzm
...
!-------------------------------------------------
  subroutine seed_bry![
    ...
    !local
    integer(kind=4) :: i,j,ip,np_old,ntag_old
    real(kind=8)    :: hvol
    real(kind=8),dimension(:),pointer:: px,py
...
!-------------------------------------------------
  subroutine seed_particles(hvol,zw,i,j,np_cur)![
    implicit none
    ! import/export
    real(kind=8)                ,intent(in) :: hvol ! volume of the water column
    real(kind=8),dimension(0:nz),intent(in) :: zw   ! w-point z-level
    integer(kind=4)             ,intent(in) :: i,j  ! local index position
    real(kind=8),optional       ,intent(in) :: np_cur ! particles already here
...
!       print *,'maxmin: ',maxval(part(1:np,1:7)),minval(part(1:np,1:7)),mynode
      ...
      part(1:np,ipx) = part(1:np,ipx) + (iSW_corn+0.5_8)   ! start of sub-domain is actually 0.5 rho
      part(1:np,ipy) = part(1:np,ipy) + (jSW_corn+0.5_8)   ! but locally we defined it as 0.

      call ncwrite(ncid,'particles',part(1:np,1:7),start)

      ! translate back to local indices (0-nx):
      part(1:np,ipx) = part(1:np,ipx) - (iSW_corn+0.5_8)
      part(1:np,ipy) = part(1:np,ipy) - (jSW_corn+0.5_8)
...
!-------------------------------------------------
  subroutine create_particles_file(fname)![
    ...
    integer(kind=4),          dimension(3) :: dimsizes
    ...
    if (ierr/=nf90_noerr) then
      ...
      &info="error creating particles file",&
      &context=module_name//"/"//sr_name)
    end if
...
!-------------------------------------------------
  subroutine rhs_particles![
    ...
    ! local
    integer(kind=4) :: i,j,k,iu,jv,kw,ip
    real(kind=8)    :: x,y,z,xu,yv,zw
    real(kind=8)    :: pdxi,pdyi,pdz
    real(kind=8),dimension(:),pointer:: px,py,pz
    real(kind=8),dimension(:),pointer:: pu,pv,pw
    real(kind=8),dimension(:),pointer:: prx,pry,prz

    real(kind=8), dimension(2,2)   :: tmp2d   ! DPD temp to remove debug warnings
    real(kind=8), dimension(2,2,2) :: tmp3d
...
!     enddo
    ...
    do ip = 1,np
      if (pz(ip)< 2*nz) then
        ...
        k = max(k,1)
        k = min(k,nz-1)
        iu= floor(px(ip)+1.0_8)
        jv= floor(py(ip)+1.0_8)
        kw= floor(pz(ip))
        ...
        if (px(ip)<-0.5_8.or.px(ip)>nx+0.5_8) then
          print *,'px: ', ip,px(ip),mynode
          stop
        endif
        if (py(ip)<-0.5_8.or.py(ip)>ny+0.5_8) then
          print *,'py: ', ip,py(ip),mynode
          stop
        endif
        if (pz(ip)< 0.0_8.or.pz(ip)>nz) then
...
!     local
    ...
    if (mynode<3) then
      do k = 1,nz
        do j = -1,ny+2
          do i = -1,nx+2
            z = z_r(i,j,k)
            u(i,j,k,:) = 0.1_8*(tanh(0.5_8*(z+30))+1)
            v(i,j,k,:) = 0._8
          enddo
        enddo
      enddo
    ...
    do k = 1,nz
      do j = -1,ny+2
        do i = -1,nx+2
          FlxU(i,j,k) = 0.5_8*(Hz(i,j,k)+Hz(i-1,j,k))*dn_u(i,j)*u(i,j,k,1)
          FlxV(i,j,k) = 0.5_8*(Hz(i,j,k)+Hz(i,j-1,k))*dm_v(i,j)*v(i,j,k,1)
        enddo
      enddo
    enddo
    Wp(:,:,0) = 0
...
!------------------------------------------
  subroutine remsort(val,idx,val_mx) ![
    implicit none
    !import/export
    integer(kind=4)                      ,intent(in)    :: val_mx
    real(kind=8)   ,dimension(:)         ,intent(inout) :: val
    integer(kind=4),dimension(:),optional,intent(inout) :: idx
    !local
    integer(kind=4) :: i,i2,ierr,i3
...
!------------------------------------------
  subroutine sort_particles  ![
    ...
    ! local
    integer(kind=4) :: ip,iv,val_mx,ierr,np_old
...
! ---------------------------------------------------------------------
  subroutine exchange_data  ![
    ...
    use roms_mpi, only:&
    &p_e, p_n, p_ne, p_nw, p_s, p_se, p_sw,&
    &p_w
    ...
    ! local
    integer(kind=4) :: i, j, ierr, mess_count, comm(16), req(16), status(MPI_STATUS_SIZE)
!# ifdef ALT_SEND_ORD
!      integer :: ipass
!# endif
    logical :: flag
    ...
    ! ocean_grid_comm = MPI_COMM_WORLD is the group of all mpi pr
    if (p_e>=0) then                              ! req() stores the mpi request handle to later use
...
!     if (mess_count>0) print *, mynode, 'mess_count=',mess_count
    ...
    do while (mess_count>0)
      ...
      do j=9,16
        if (comm(j) > 0) then
          call MPI_Test (req(j), flag, status, ierr)         ! make sure all sent messages have been received elsewher
...
! ---------------------------------------------------------------------
  recursive subroutine qsort(a, d, first, last)  ![
    ...
    integer(kind=4),dimension(:),optional,intent(inout) :: d
...
!------------------------------------------
  subroutine zlev2kidx(zlev,kidx,zw)![
    implicit none
    ! import/export
    real(kind=8),dimension(:)   ,intent(inout):: zlev
    real(kind=8),dimension(0:nz),intent(in)   :: zw
    real(kind=8),dimension(:)   ,intent(out)  :: kidx
    ! local
    integer(kind=4) :: i,k,npz
