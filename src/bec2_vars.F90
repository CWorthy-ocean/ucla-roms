...
#include "cppdefs.opt"
  ...
  real(kind=8),allocatable,dimension(:,:,:,:) :: DTRACER_MODULE

!      integer, pointer :: nr_bec2_diag_2d, nr_bec2_diag_3d, nr_bec2_diag
!      integer, pointer :: nr_bec2_wrdiag_2d, nr_bec2_wrdiag_3d, nr_bec2_wrdiag

  real(kind=8),dimension(:,:,:,:),pointer   :: bec2_diag_3d
  real(kind=8),dimension(:,:,:)  ,pointer   :: bec2_diag_2d

! Control BEC2_DIAG Output vars
  logical, dimension(:), pointer :: wrt_bec2_diag_2d
  logical, dimension(:), pointer :: wrt_bec2_diag_3d
  integer(kind=4), dimension(:), pointer :: idx_bec2_diag_2d
  integer(kind=4), dimension(:), pointer :: idx_bec2_diag_3d

  character(len=72), pointer ::  vname_bec2_diag_2d(:,:)
  character(len=72), pointer ::  vname_bec2_diag_3d(:,:)

#if defined(BEC2_DIAG)
  ! Indices to be used in bec2_diag_3d only:
  integer(kind=4) :: par_idx_t, pocfluxin_idx_t,&
  ...
  integer(kind=4) :: pco2air_idx_t, parinc_idx_t, fgo2_idx_t, fgco2_idx_t, ws10m_idx_t, xkw_idx_t,&
...
!     &           slavgT_bec2_diag(nr_bec2_diag), rstT_bec2_diag(nr_bec2_diag)
  integer(kind=4), parameter, public :: nr_bec2_diag_2d = 37 !TODO clean
  integer(kind=4), parameter, public :: nr_bec2_diag_3d = 103 !TODO clean
!
#endif /* BEC2_DIAG */


  real(kind=8),allocatable,dimension(:,:) :: ifrac, press
...
!!
  real(kind=8),allocatable,dimension(:,:) :: ph_srf
