...
# define SMOOTH_RIG  /* local Ri number smoothing */
  ...
  subroutine lmd_vmix(tind)
    ...
    use private_scratch, only: a3d, a2d, ia2d
...
!             parameterization.  Reviews of Geophysics, 32,363-403._8
    ...
    &nu0m= 1.D-2,&     ! Maximum viscosity and diffusivity
    &nu0s= 1.D-2,&     ! due to shear instability [m^2/s];

    &nuwm=1.0D-4,&     ! Interior viscosity and diffusivity
    &nuws=0.1D-4,&     ! due to wave breaking, [m^2/s]

    &nu0c=0.1_8,&        ! Convective adjustment [m^2/s];

    &lmd_nu=1.5D-6,&   ! Molecular viscosity [m^2/s];

    ! Value of double-diffusive density
    &lmd_Rrho0=1.9_8,&   ! ratio where diffusivity goes to zero
    !                     in salt fingering.
    &lmd_nuf=10.0D-4,& ! Scaling factors for double diffusion
    &lmd_fdd=0.7_8,&     ! coefficient in salt fingering.

    &lmd_tdd1=0.909_8,&
    &lmd_tdd2=4.6_8,&    ! Double diffusion constants for
    &lmd_tdd3=0.54_8,&   ! temperature (Marmorino and Caldwell,
    &lmd_sdd1=0.15_8,&   ! 1976) and salinity (Fedorov, 1988)
    &lmd_sdd2=1.85_8,&
    &lmd_sdd3=0.85_8,&
    &eps=1.D-14,&
    &Lturb = 10._8      ! Length scale of bottom boundary layer
    ! where turbulence is supressed

    real(kind=8) dist,mult

# ifdef LMD_DDMIX
    real(kind=8) Rrho, ddDS, ddDT, nu_dds, nu_ddt, alfaobeta, Tt, Ts, Tp
    real(kind=8), parameter ::   A0=+0.665157D-01,    A1=+0.170907D-01,&
    &A2=-0.203814D-03,  A3=+0.298357D-05,  A4=-0.255019D-07,&
    &B0=+0.378110D-02,  B1=-0.846960D-04,  C0=-0.678662D-05,&
    &D0=+0.380374D-04,  D1=-0.933746D-06,  D2=+0.791325D-08,&
    &E0=-0.164759D-06,  F0=-0.251520D-11,  G0=+0.512857D-12,&
    &H0=-0.302285D-13,  Smean=35.0_8
# endif
...
do j=jstr,jend
  do i=istr,iend
# ifdef LMD_RIMIX
    cff=min(1._8, max(0._8, Rig(i,j,k)))  ! Compute mixing due
    nu_sx=1._8 - cff*cff                ! to shear instability
...
do j=jstr,jend
  do i=istr,iend
    Tt=0.5_8*(t(i,j,k,tind,itemp)+t(i,j,k+1,tind,itemp))
    Ts=0.5_8*(t(i,j,k,tind,isalt)+t(i,j,k+1,tind,isalt))&
    &-Smean
    Tp=-z_w(i,j,k)
    ...
    ddDS=sign(1._8,ddDS)*max(abs(ddDS),eps)
...
!-------------------------------
    ...
    elseif (Rrho<1._8 .and. Rrho>0._8 .and.&
    &ddDS<0._8) then

! Compute interior diffusivity for double diffusive mixing of
! temperature (Marmorino and Caldwell, 1976); (lmd_nu=1.5D-6,
! lmd_tdd1=0.909_8, lmd_tdd2=4.6_8, lmd_tdd3=0.54_8).

      nu_ddt=lmd_nu*lmd_tdd1*&
      &exp(lmd_tdd2*exp(-lmd_tdd3*((1._8/Rrho)-1._8)))

! Compute interior diffusivity for double diffusive mixing
! of salinity (lmd_sdd1=0.15_8, lmd_sdd2=1.85_8, lmd_sdd3=0.85_8).
...
do k=1,N-1
  do j=jstr,jend
    do i=istr,iend
      ...
      if (dist<Lturb) then
        mult = sin(0.5_8*pi*(z_w(i,j,k) - z_w(i,j,0))/Lturb)
...
do k=1,N-1
  do j=jstr,jend
    do i=istr,iend
!           Akv(i,j,k)=Kv(i,j,k)
      AKv(i,j,k)=0.5_8*Kv(i,j,k)+0.25_8*Kv(i,j,k-1)+0.25_8*Kv(i,j,k+1) + Akv_bak
!           Akt(i,j,k,itemp)=Kt(i,j,k)
      AKt(i,j,k,itemp)=0.5_8*Kt(i,j,k)+0.25_8*Kt(i,j,k-1)+0.25_8*Kt(i,j,k+1) + Akt_bak(itemp)
#  ifdef SALINITY
!           Akt(i,j,k,isalt)=Ks(i,j,k)
      AKt(i,j,k,isalt)=0.5_8*Ks(i,j,k)+0.25_8*Ks(i,j,k-1)+0.25_8*Ks(i,j,k+1) + Akt_bak(isalt)
#  endif
    enddo
  enddo
enddo
