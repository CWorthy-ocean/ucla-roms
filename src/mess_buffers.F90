...
! margins for up to 4 full 3D arrays to be exchanged simultaneously.
  ...
  integer(kind=4) :: size_Z, size_X, size_E

  real(kind=8),allocatable,dimension(:) :: sn_NW, sendN, sn_NE, rv_NW, recvN, rv_NE,&
  &sendW, sendE, recvW, recvE, sn_SW, sendS,&
  &sn_SE, rv_SW, recvS, rv_SE

!$ OMP THREADPRIVATE(sn_NW,rv_NW, sendN,recvN, sn_NE,rv_NE)
!$ OMP THREADPRIVATE(sendW,recvW, sendE,recvE)
!$ OMP THREADPRIVATE(sn_SW,rv_SW, sendS,recvS, sn_SE,rv_SE)
