...
!-----------------------------------------------------------------------
    ...
    integer(kind=4)            :: len1, len2, stat1, stat2

! --- SLURM ---
    call get_environment_variable('SLURM_JOB_ID', jobid, len1, stat1)
    call get_environment_variable('SLURM_ARRAY_TASK_ID', taskid,&
    &len2, stat2)

    if (stat1 == 0) then
      mpi_master_only write(*,'(A,1X,A)') 'SLURM Job ID:',&
      &trim(jobid(1:len1))
      if (stat2 == 0) then
        mpi_master_only write(*,'(A,1X,A)') 'SLURM Array Task ID:',&
        &trim(taskid(1:len2))
      end if
      return
    end if
! --- PBS ---
    ...
    call get_environment_variable('PBS_ARRAY_INDEX', taskid, len2,&
    &stat2)

    if (stat1 == 0) then
      mpi_master_only write(*,'(A,1X,A)') 'PBS Job ID:',&
      &trim(jobid(1:len1))
      if (stat2 == 0) then
        mpi_master_only write(*,'(A,1X,A)') 'PBS Array Index:',&
        &trim(taskid(1:len2))
      end if
      return
    end if
