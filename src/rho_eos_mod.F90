...
! computed, and
!
!     K(T,S,|z|) = K00 + K01(T,S) + K1(T,S)*|z| + K2(T,S)*|z|^2._8  (2)
!
...
!
!                   rho1 - rho0*[K01+K1*|z|+K2*|z|^2]/[K00-0.1_8|z|]
!    rho1 + 0.1_8|z| -----------------------------------------------
!                        K00 + K01 + (K1-0.1_8)*|z| + K2*|z|^2
!                                                                 (4)
...
!             Stability. J. Atmos. Ocean. Tec., vol. 12, pp. 381-389._8
!
...
!                                          John Wilkin, 29 July 92
    ...
    use tracers, only: t
    use coupling, only: rhos, rhoa
    use dimensions, only: npx, npy
#ifdef SPLIT_EOS
    use eos_vars, only: qp2, rho1, qp1
#else
    ...
    use grid, only: rmask
    use param, only: ieast, iwest, jnorth, jsouth, itemp, isalt
# if defined LMD_MIXING  || defined LMD_KPP || defined MY25_MIXING || defined LMD_BKPP
    use mixing, only: bvf
#endif
...
! temperature powers, "S" for 3/2 power of salinity.
    ...
    real(kind=8), parameter ::   r00=999.842594_8,     r01=6.793952D-2,&
...
#  ifndef SPLIT_EOS
    real(kind=8), parameter ::   B00=0.4721788_8,&
    &B01=0.01028859_8,     B02=-2.512549D-4,   B03=-5.939910D-7,&
    &B10=-0.01571896_8,    B11=-2.598241D-4,   B12=7.267926D-6,&
    &BS1=2.042967D-3,&
...
#  ifdef SALINITY
          Ts=t(i,j,k,tidx,isalt) ; sqrtTs=sqrt(max(0._8,Ts))
#  endif
...
#   ifdef SALINITY
          rho1(i,j,k)=rho1(i,j,k) + Scoef*t(i,j,k,tidx,isalt)
#   endif
          qp1(i,j,k)=0._8
#  else
          rho(i,j,k)=cff  -Tcoef*t(i,j,k,tidx,itemp)
#   ifdef SALINITY
          rho(i,j,k)=rho(i,j,k)+Scoef*t(i,j,k,tidx,isalt)
#   endif
#  endif
