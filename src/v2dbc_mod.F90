...
! integrated) ETA-component velocity vbar(:,:,knew).
    ...
    use boundary, only:&
    &zeta_south, vbar_south, zeta_north,&
    &vbar_north, vbar_west, vbar_east
    use grid, only: h, pn, vmask, pm
    use ocean_vars, only: zeta, vbar, ubar
    use scalars, only: dtfast, g, knew, kstp, gamma2

    implicit none
    integer(kind=4) istr,iend,jstr,jend, i,j
    real(kind=8) grad(PRIVATE_2D_SCRATCH_ARRAY), cx,cy, cext, cff,&
    &dft,dfx,dfy, zx,hx
...
#  ifdef OBC_M2FLATHER
      do i=istr,iend
        ...
        if (cx > 0.292893218813452_8) then
...
#   ifdef OBC_RAD_NORMAL
        cy=0._8 ; cff=max(dfx*dfx, eps)
#   else
        if (dft*(grad(i,jstr+1)+grad(i+1,jstr+1)) > 0._8) then
          dfy=grad(i  ,jstr+1)
        else
          dfy=grad(i+1,jstr+1)
        endif
...
#   endif
        ...
        cx=dft*dfx
        if (cx < 0._8) then       !--> cancel, if inflow
          cx=0._8 ; cy=0._8
#   if defined M2_FRC_BRY || defined M2NUDGING
          cext=dtfast*ubind * 0.5_8*(pn(i,jstr-1)+pn(i,jstr))
        else
          cext=0._8
#   endif
        endif
...
#   endif
#   if defined M2_FRC_BRY || defined M2NUDGING
        vbar(i,jstr,knew)=(1._8-cext)*vbar(i,jstr,knew)&
#    ifdef M2_FRC_BRY
        &+cext*vbar_south(i)
#    else
        &+cext*vbclm(i,jstr)&
#    endif
        &-attnM2*dtfast*g*0.5_8*(pn(i,jstr-1)+pn(i,jstr))&
#    ifdef M2_FRC_BRY
...
# else
      ...
      do i=istr,iend                       ! ======== ==== ======
        vbar(i,jstr,knew)=0._8               ! (no-flux, default)
      enddo
# endif          /* OBC_SOUTH */
    endif   !<-- SOUTHERN_EDGE
...
#   ifdef OBC_RAD_NORMAL
        cy=0._8 ; cff=max(dfx*dfx, eps)
#   else
        if (dft*(grad(i,jend)+grad(i+1,jend)) > 0._8) then
          dfy=grad(i  ,jend)
        else
          dfy=grad(i+1,jend)
        endif
...
#   endif
        ...
        if (cx < 0._8) then       !--> cancel, if inflow
          cx=0._8 ; cy=0._8
#   if defined M2_FRC_BRY || defined M2NUDGING
          cext=dtfast*ubind * 0.5_8*(pn(i,jend)+pn(i,jend+1))
        else
          cext=0._8
#   endif
        endif
...
#   endif
#   if defined M2_FRC_BRY || defined M2NUDGING
        vbar(i,jend+1,knew)=(1._8-cext)*vbar(i,jend+1,knew)&
#    ifdef M2_FRC_BRY
        &+cext*vbar_north(i)
#    else
        &+cext*vbclm(i,jend+1)&
#    endif
        &+attnM2*dtfast*g*0.5_8*(pn(i,jend)+pn(i,jend+1))&
#    ifdef M2_FRC_BRY
...
# else
      do i=istr,iend                        ! Northern edge closed
        vbar(i,jend+1,knew)=0._8                ! (no-flux, default)
      enddo
# endif /* OBC_NORTH */
    endif          !<--  NORTHERN_EDGE
#endif                  /* !NS_PERIODIC */
...
#    else
        if (dft*(grad(istr,j-1)+grad(istr,j)) > 0._8) then
          dfy=grad(istr,j-1)
        else
          dfy=grad(istr,j  )
        endif
...
#    endif
        ...
        if (cx < 0._8) then       !--> cancel, if inflow
          cx=0._8 ; cy=0._8
#    if defined M2_FRC_BRY || defined M2NUDGING
          cext=dtfast*ubind * 0.5_8*(pm(istr-1,j-1)+pm(istr-1,j))
        else
          cext=0._8
#    endif
        endif
...
#   if defined M2_FRC_BRY || defined M2NUDGING
        vbar(istr-1,j,knew)=(1._8-cext)*vbar(istr-1,j,knew)&
#    ifdef M2_FRC_BRY
        &+cext*vbar_west(j)
#    else
        &+cext*vbclm(istr-1,j)
...
#  else          /* alternative open: Chapman, specified, gradient */
      do j=jstrV,jend
#   if defined OBC_M2FLATHER
        cx=dtfast*sqrt(0.5_8*g*(h(istr-1,j-1)+h(istr-1,j)))&
        &*0.5_8*(pm(istr-1,j-1)+pm(istr-1,j))
        vbar(istr-1,j,knew)=( vbar(istr-1,j,kstp)&
        &+cx*vbar(istr,j,knew) )/(1._8+cx)
#   elif defined OBC_M2SPECIFIED
#    ifdef M2_FRC_BRY
...
#    ifdef OBC_RAD_NORMAL
      cy=0._8 ; cff=max(dfx*dfx, eps)
#    else
      if (dft*(grad(iend,j-1)+grad(iend,j)) > 0._8) then
        dfy=grad(iend,j-1)
      else
        dfy=grad(iend,j  )
      endif
...
#    endif
      ...
      if (cx < 0._8) then       !--> cancel, if inflow
        cx=0._8 ; cy=0._8
#    if defined M2_FRC_BRY || defined M2NUDGING
        cext=dtfast*ubind * 0.5_8*(pm(iend+1,j-1)+pm(iend+1,j))
      else
        cext=0._8
#    endif
      endif
...
#   if defined M2_FRC_BRY || defined M2NUDGING
      vbar(iend+1,j,knew)=(1._8-cext)*vbar(iend+1,j,knew)&
#    ifdef M2_FRC_BRY
      &+cext*vbar_east(j)
#    else
      &+cext*vbclm(iend+1,j)
...
#  else          /* alternative open: Chapman, specified, gradient */
    do j=jstrV,jend
#   if defined OBC_M2FLATHER
      cx=dtfast*sqrt(0.5_8*g*(h(iend+1,j-1)+h(iend+1,j)))&
      &*0.5_8*(pm(iend+1,j-1)+pm(iend+1,j))
      vbar(iend+1,j,knew)=( vbar(iend+1,j,kstp)&
      &+cx*vbar(iend,j,knew))/(1._8+cx)
#   elif defined OBC_M2SPECIFIED
#    ifdef M2_FRC_BRY
