...
! ----------------------------------------------------------------------
  subroutine remap_src_to_grid(N_src, H_src, t_src, N_tgt, H_tgt, t_tgt)
    ...
    integer(kind=4), intent(in)                 :: N_src
    real(kind=8), dimension(N_src), intent(in)  :: H_src
    real(kind=8), dimension(N_src), intent(in)  :: t_src
    integer(kind=4), intent(in)                 :: N_tgt
    real(kind=8), dimension(N_tgt), intent(in)  :: H_tgt
    real(kind=8), dimension(N_tgt), intent(out) :: t_tgt

    ! local
    integer(kind=4) :: k,k_new,trc,idx
    real(kind=8), dimension(N_src+1) :: interface_values
    real(kind=8) :: total_depth_src, total_depth_tgt
    real(kind=8), dimension(N_src) :: a0, a1, a2
    real(kind=8), dimension(N_src+1) :: z_orig_interfaces
    real(kind=8), dimension(N_src) :: H_orig
    real(kind=8) :: H_src_tot

    integer(kind=4) :: curr_src_index
    integer(kind=4), dimension(N_tgt) :: tgt_index_start
    integer(kind=4), dimension(N_tgt) :: tgt_index_end

    ! Keep track of how far up we are in the water column
    real(kind=8) :: curr_tgt_height, curr_src_height

    ! Keep track of where in each H_orig cell the interfaces of H_tgt are
    real(kind=8), dimension(N_tgt) :: tgt_frac_start
    real(kind=8), dimension(N_tgt) :: tgt_frac_end

    ! This holds the integrated_values
    real(kind=8), dimension(N_tgt) :: definite_integral

    ! Temporary array to hold the remapped fluxes
    real(kind=8), dimension(N_tgt) :: t_tmp

    ! Scalars to hold the tracer mass in each cell
    real(kind=8) :: total_t_src
    real(kind=8) :: total_t_tgt
    real(kind=8) :: total_t_diff
    real(kind=8) :: cellwise_mass_frac


    tgt_frac_start(:) = 0.0_8
    tgt_frac_end(:)   = 0.0_8

    definite_integral(:) = 0.0_8
    t_tmp(:) = 0.0_8
    t_tgt(:) = 0.0_8
    ...
    do k = 1,N_tgt
      ...
      do idx = tgt_index_start(k),tgt_index_end(k)
        ...
        elseif ((idx == tgt_index_start(k)) .and. (idx < tgt_index_end(k))) then
          definite_integral(k) = integrate(a0(idx), a1(idx), a2(idx), tgt_frac_start(k), 1.0_8)*H_src(idx)
        elseif ((idx < tgt_index_end(k)) .and. (idx > tgt_index_start(k))) then
          definite_integral(k) = definite_integral(k) + integrate(a0(idx), a1(idx), a2(idx), 0.0_8, 1.0_8)*H_src(idx)
        else
          definite_integral(k) = definite_integral(k) +&
          &integrate(a0(idx), a1(idx), a2(idx), 0.0_8, tgt_frac_end(k))*H_src(idx)
        endif
...
! ----------------------------------------------------------------------
  real(kind=8) function integrate(a0, a1, a2, z0, z1) ![
    ! Definite integration of a parabola

    implicit none

    real(kind=8) :: a0, a1, a2, z0, z1
    real(kind=8) :: one_third = 0.3333333333_8

    integrate = a0 * (z1 - z0) + 0.5_8*a1*(z1**2 - z0**2) +&
...
! ----------------------------------------------------------------------
  subroutine calc_interface_values(N_src, H, arr, int_vals)  ![
    ...
    integer(kind=4), intent(in) :: N_src
    real(kind=8), dimension(N_src), intent(in)    :: H
    real(kind=8), dimension(N_src), intent(in)    :: arr
    real(kind=8), dimension(N_src+1), intent(out) :: int_vals

    real(kind=8), dimension(0:3) :: B
    real(kind=8), dimension(0:3,0:3) :: M
    real(kind=8) :: h_b
    real(kind=8) :: h_t
    real(kind=8) :: iH
    real(kind=8) :: Ts_bot
    ...
    integer(kind=4) :: k, kk, kkp, kkp1, trc
    integer(kind=4) :: ord = 3
    ...
    do k = 0,ord
      do kk = 0,ord
        kkp1 = kk+1
        M(k,kk) = (1.0_8 / kkp1) * iH * (h_t**(kkp1) - h_b**(kkp1))
      enddo
...
! ----------------------------------------------------------------------
  real(kind=8) function gauss(M, b, ord) ![
    ...
    real(kind=8), dimension(0:3,0:3), intent(inout) :: M
    real(kind=8), dimension(0:3), intent(inout) :: b
...
! ----------------------------------------------------------------------
  subroutine thomas_PPM(N_src, Ts_bot, Ts_top, arr, H, int_vals)  ![
    ...
    integer(kind=4), intent(in) :: N_src
    real(kind=8), intent(in) :: Ts_bot
    real(kind=8), intent(in) :: Ts_top
    real(kind=8), dimension(N_src),   intent(in)  :: arr
    real(kind=8), dimension(N_src),   intent(in)  :: H
    real(kind=8), dimension(N_src+1), intent(out) :: int_vals

    integer(kind=4) :: n,t,k
    real(kind=8), dimension(N_src+1) :: a, b, c, d, cp, dp, x
    real(kind=8) :: h0, h1, alpha, beta, d1, d2
