...
#ifdef Ncycle_SY
  use tracers, only: in2, in2o
#endif
  !, only: t, iTandS
  use surf_flux, only: srflx, sustr, svstr
! , only: sustr,sustr_r,svstr,svstr_r,srflx
# ifdef BULK_FRC
...
# endif
  ...
  &fgn2o_idx_t, fgo2_idx_t, ironflux_idx_t, n2osat_idx_t,&
  &n2sat_idx_t, o2sat_idx_t, pco2air_idx_t, pco2oc_idx_t, ph_idx_t,&
  &pvco2_idx_t, pvn2_idx_t, pvn2o_idx_t, pvo2_idx_t,&
  &schmidt_n2_idx_t, schmidt_n2o_idx_t, schmidtco2_idx_t,&
  &schmidto2_idx_t, ws10m_idx_t, xkw_idx_t, wrt_bec2_diag_2d,&
  &idx_bec2_diag_2d,
#endif
  &ifrac, landmask, press, ph_srf,&
  &dtracer_module, lflux_gas_co2
  use bgc_forces, only: &
...
# endif
  ...
  integer(kind=4) su_i1, su_i2, sv_j1, sv_j2
...
# endif
  ...
  real(kind=8),dimension(istr:iend,jstr:jend)::&
...
# endif
  ...
  &phlo_init = 7.0_8,&  ! low bound for ph for no prev soln
  &phhi_init = 9.0_8,&  ! high bound for ph for no prev soln
  &del_ph = 0.20_8     ! delta-ph for prev soln

  real(kind=8), PARAMETER ::&
  &a = 8.6D-7       ! a = 0.31 cm/hr s^2/m^2 in (s/m)
  real(kind=8),dimension(istr:iend)::&
...
!
  do j=jstr,jend
    ...
    do i=istr,iend
      ...
      sustr_rho(i,j) = 0.5_8*(sustr(su_i1,j)+sustr(su_i2,j))
      svstr_rho(i,j) = 0.5_8*(svstr(i,sv_j1)+svstr(i,sv_j2))
    end do
  end do
# endif
...
# endif
  ...
  !----------------------------------------------------------------------
  WHERE (AP_USED(istr:iend,jstr:jend) > 1.5_8 .OR. AP_USED(istr:iend,jstr:jend) < 0.5_8)
...
# endif
  ...
  IF (lflux_gas_o2 .OR. lflux_gas_co2) THEN
    ...
    WHERE (FICE_USED(istr:iend,jstr:jend) > 0.2_8 .AND. FICE_USED(istr:iend,jstr:jend) < 0.9999_8)
      XKW_ICE(istr:iend,jstr:jend) = (c1 - FICE_USED(istr:iend,jstr:jend)) * XKW_ICE(istr:iend,jstr:jend)
    END WHERE
    WHERE (FICE_USED(istr:iend,jstr:jend) >= 0.9999_8)
...
!        lflux_gas_n2o = .true.
  IF (lflux_gas_n2o) THEN
    ...
    do j=jstr,jend
      do i=istr,iend
        PV(i,j) = XKW_ICE(i,j) * SQRT(660.0_8 / SCHMIDT_USED(i,j))&
# ifdef MASKING
        &* rmask(i,j)
# endif
        XN2O = 300.0_8 * 1e-9 !!! need to figure out namelists here
...
# if defined BEC2_DIAG && defined Ncycle_SY
        if (wrt_bec2_diag_2d(fgn2o_idx_t)) then
          ...
          bec2_diag_2d(i,j,idiag) = STF(i,j,bgc_idx(iN2O))
        endif
...
# ifdef MASKING
        &* rmask(i,j)
# endif
        N2SAT_USED = AP_USED(i,j) * N2SAT_1atm(i,j) * 0.78084_8& !Molar ratio of N2 in air
# ifdef MASKING
        &* rmask(i,j)
# endif
...
&qa_dust_def,istr,iend,j,dt2,dust_flux)
  ...
  use grid, only: rmask
  use ocean_vars, only: hz, z_r
  use scalars, only: dt, nnew
  use bgc_shared_vars, only: bgc_idx
  use bec2_params, only:&
  &autotroph_cnt, c0, c1, c10, c2,&
  &caco3_poc_min, caco3_sp_thres, caco3_temp_thres1,&
  &caco3_temp_thres2, denitrif_c_n, denitrif_n2o_c, denitrif_no2_c,&
  &denitrif_no3_c, diat_ind, diaz_ind, doc_reminr, dofe_reminr,&
  &don_reminr, donr_reminr, donrefract, dop_reminr, dopr_reminr,&
  &doprefract, dps, dust_fescav_scale, f_graze_caco3_remin,&
  &f_graze_si_remin, f_graze_sp_poc_lim, f_photosp_caco3,&
  &f_qsw_par, fe_max_scale2, fe_scavenge_thres1, gqsi_0, gqsi_max,&
  &gqsi_min, loss_thres_zoo, par_thres_pchl, parm_f_prod_sp_caco3,&
  &parm_fe_scavenge_rate0, parm_denitrif_NO3_limit,&
#ifdef Ncycle_SY
  &parm_kao, parm_kax, parm_kden1,&
  &parm_kden2, parm_kden3, parm_kn2o_den3, parm_knh4_ao,&
  &parm_knh4_ax, parm_kno, parm_kno2_ax, parm_kno2_den2,&
  &parm_kno2_no, parm_kno3_den1, parm_ko2_ao, parm_ko2_ax,&
  &parm_ko2_den1, parm_ko2_den2, parm_ko2_den3, parm_ko2_no,&
  &parm_ko2_oxic, parm_koxic, parm_n2o_ji_a,&
  &parm_n2o_ji_b, r_no2tonh4_ax, parm_red_d_c_o2_no2v,&
#endif
  &parm_labile_ratio,  parm_nitrif_par_lim, parm_o2_min,&
  &parm_o2_min_delta, parm_red_d_c_o2, parm_red_d_c_o2_diaz,&
  &parm_remin_d_c_o2, parm_z_mort2_0, parm_kappa_nitrif,&
  &parm_z_mort_0, q, q_10, qcaco3_max, qfe_zoo, qp_zoo_pom,&
  &r_nfix_photo, sp_ind, spc_poc_fac, t0_kelvin,&
  &thres_z1, thres_z2, tref, yps, ksio3, gqfe_0, kfe, gqfe_min,&
  &caco3_ind, temp_thres, loss_thres2, loss_thres, knh4, kno2,&
  &kno3, nfixer, kdop, kpo4, pcref, alphapi, qp, thetan_max,&
  &imp_calcifier, exp_calcifier, mort, agg_rate_max, mort2,&
  &agg_rate_min, grazee_ind, z_umax_0, z_grz, graze_zoo, graze_poc,&
  &graze_doc, loss_poc, f_zoo_detr, c_ind, chl_ind, fe_ind, si_ind
  use bec2_vars, only:&
#ifdef BEC2_DIAG
  &bec2_diag_3d, bec2_diag_2d, ammox_idx_t,&
  &anammox_idx_t, caco3prod_idx_t, denitrif1_idx_t,&
  &denitrif2_idx_t, denitrif3_idx_t, diatagg_idx_t,&
  &diatczero_idx_t, diatfeuptake_idx_t, diatlightlim_idx_t,&
  &diatloss_idx_t, diatnh4uptake_idx_t, diatnlim_idx_t,&
  &diatno2uptake_idx_t, diatno3uptake_idx_t, diatphotoacc_idx_t,&
  &diatplim_idx_t, diatpo4uptake_idx_t, diatsio3uptake_idx_t,&
  &diatsiuptake_idx_t, diazagg_idx_t, diazczero_idx_t,&
  &diazfeuptake_idx_t, diazlightlim_idx_t, diazloss_idx_t,&
  &diaznfix_idx_t, diaznh4uptake_idx_t, diazno2uptake_idx_t,&
  &diazno3uptake_idx_t, diazphotoacc_idx_t, diazplim_idx_t,&
  &diazpo4uptake_idx_t, docprod_idx_t, docremin_idx_t,&
  &doczero_idx_t, dofeprod_idx_t, doferemin_idx_t, donprod_idx_t,&
  &donremin_idx_t, donrremin_idx_t, dopprod_idx_t, dopremin_idx_t,&
  &fescavenge_idx_t, fescavengerate_idx_t, grazediat_idx_t,&
  &grazediatzoo_idx_t, grazediaz_idx_t, grazediazzoo_idx_t,&
  &grazedicdiat_idx_t, grazedicdiaz_idx_t, grazedicsp_idx_t,&
  &grazesp_idx_t, grazespzoo_idx_t, ironuptakediat_idx_t,&
  &ironuptakediaz_idx_t, ironuptakesp_idx_t, lossdicdiat_idx_t,&
  &lossdicdiaz_idx_t, lossdicsp_idx_t, nitrox_idx_t, o2cons_idx_t,&
  &o2prod_idx_t, par_idx_t, parinc_idx_t, pcaco3prod_idx_t,&
  &photocdiat_idx_t, photocdiaz_idx_t, photocsp_idx_t,&
  &pironprod_idx_t, pocprod_idx_t, sio2prod_idx_t, spagg_idx_t,&
  ...
  &totphytoc_idx_t, totprod_idx_t, zooczero_idx_t, zooloss_idx_t,&
  &zoolossdic_idx_t, wrt_bec2_diag_3d, idx_bec2_diag_3d,&
  &wrt_bec2_diag_2d, idx_bec2_diag_2d, &
#endif
  &dop_remin, dopr_remin, poc_sflux_out,&
...
#ifdef Ncycle_SY
  use tracers, only: in2, in2o, ino2
#endif
  use tracers, only:&
  &t, ialk, idiatc, idiatchl, idiatfe, idiatsi,&
  &idiazc, idiazchl, idiazfe, idic, idoc, idofe, idon, idonr, idop,&
  &idopr, ife, inh4, ino3, io2, ipo4, isio3, ispc,&
  &ispcaco3, ispchl, ispfe, izooc

  implicit none

  integer(kind=4) istr,iend,i,j,k,n_loc,auto_ind,auto_ind2
  integer(kind=4) idiag
  real(kind=8) dt2,tmp1,tmp2,tmp3,tmp4
  real(kind=8) temp(istr:iend), SHF_QSW(istr:iend),&
  &PAR_out(istr:iend), dust_flux(istr:iend)

#if defined DAILYPAR_PHOTOINHIBITION
  real(kind=8) SHF_QSW_AVG(istr:iend), PAR_out_AVG(istr:iend)
#endif

  REAL(kind=8), PARAMETER ::&
  &epsC      = 1.00D-8,&  ! small C concentration (mmol C/m^3)
  &epsTinv   = 3.17D-8,&  ! small inverse time scale (1/year) (1/sec)
  &epsnondim = 1.00D-6,&  ! small non-dimensional number (non-dim)
  &cks = 9.0_8,&         ! constant used in Fe quota modification
  &cksi = 5.0_8         ! constant used in Si quota modification
...
# endif
  ...
  REAL(kind=8), DIMENSION(istr:iend,autotroph_cnt) ::&
...
# endif
  ...
  !---------------------------------------------------------------------------
  PO4_loc  = MAX(c0, T(istr:iend,j,k,nnew,iPO4))
  NO3_loc  = MAX(c0, T(istr:iend,j,k,nnew,iNO3))
  SiO3_loc = MAX(c0, T(istr:iend,j,k,nnew,iSIO3))
  NH4_loc  = MAX(c0, T(istr:iend,j,k,nnew,iNH4))
  Fe_loc   = MAX(c0, T(istr:iend,j,k,nnew,iFE))
  O2_loc   = MAX(c0, T(istr:iend,j,k,nnew,iO2))
  DOC_loc  = MAX(c0, T(istr:iend,j,k,nnew,iDOC))
  spC_loc  = MAX(c0, T(istr:iend,j,k,nnew,iSPC))
  spChl_loc= MAX(c0, T(istr:iend,j,k,nnew,iSPCHL))
  spCaCO3_loc  = MAX(c0,T(istr:iend,j,k,nnew,iSPCACO3))
  diatC_loc= MAX(c0, T(istr:iend,j,k,nnew,iDIATC))
  diatChl_loc  = MAX(c0, T(istr:iend,j,k,nnew,iDIATCHL))
  zooC_loc  = MAX(c0, T(istr:iend,j,k,nnew,iZOOC))
  spFe_loc  = MAX(c0, T(istr:iend,j,k,nnew,iSPFE))
  diatSi_loc = MAX(c0, T(istr:iend,j,k,nnew,iDIATSI))
  diatFe_loc  = MAX(c0, T(istr:iend,j,k,nnew,iDIATFE))
  diazC_loc    = MAX(c0, T(istr:iend,j,k,nnew,iDIAZC))
  diazChl_loc  = MAX(c0, T(istr:iend,j,k,nnew,iDIAZCHL))
  diazFe_loc   = MAX(c0, T(istr:iend,j,k,nnew,iDIAZFE))
  DON_loc  = MAX(c0, T(istr:iend,j,k,nnew,iDON))
  DOFe_loc = MAX(c0, T(istr:iend,j,k,nnew,iDOFE))
  DOP_loc  = MAX(c0, T(istr:iend,j,k,nnew,iDOP))
  DOPr_loc = MAX(c0, T(istr:iend,j,k,nnew,iDOPR))
  DONr_loc = MAX(c0, T(istr:iend,j,k,nnew,iDONR))
# ifdef Ncycle_SY
  NO2_loc  = MAX(c0, T(istr:iend,j,k,nnew,iNO2))
  N2_loc  = MAX(c0, T(istr:iend,j,k,nnew,iN2))
  N2O_loc = MAX(c0, t(istr:iend,j,k,nnew,iN2O))
# endif

#ifdef BEC2_DIAG
  if (wrt_bec2_diag_3d(totchl_idx_t)) then
    idiag = idx_bec2_diag_3d(totchl_idx_t)
    bec2_diag_3d(istr:iend,j,k,idiag) = t(istr:iend,j,k,nnew,iSPCHL)&
    &+ t(istr:iend,j,k,nnew,iDIATCHL) + t(istr:iend,j,k,nnew,iDIAZCHL)
  endif
  if (wrt_bec2_diag_3d(totphytoc_idx_t)) then
    idiag = idx_bec2_diag_3d(totphytoc_idx_t)
    bec2_diag_3d(istr:iend,j,k,idiag) = t(istr:iend,j,k,nnew,iSPC)&
    &+ t(istr:iend,j,k,nnew,iDIATC) + t(istr:iend,j,k,nnew,iDIAZC)
  endif
  if (wrt_bec2_diag_3d(spczero_idx_t)) then
    idiag = idx_bec2_diag_3d(spczero_idx_t)
    bec2_diag_3d(istr:iend,j,k,idiag) = max(c0,-T(istr:iend,j,k,nnew,iSPC))/dt
  endif
  if (wrt_bec2_diag_3d(diatczero_idx_t)) then
    idiag = idx_bec2_diag_3d(diatczero_idx_t)
    bec2_diag_3d(istr:iend,j,k,idiag) = max(c0,-T(istr:iend,j,k,nnew,iDIATC))/dt
  endif
  if (wrt_bec2_diag_3d(diazczero_idx_t)) then
    idiag = idx_bec2_diag_3d(diazczero_idx_t)
    bec2_diag_3d(istr:iend,j,k,idiag) = max(c0,-T(istr:iend,j,k,nnew,iDIAZC))/dt
  endif
  if (wrt_bec2_diag_3d(doczero_idx_t)) then
    idiag = idx_bec2_diag_3d(doczero_idx_t)
    bec2_diag_3d(istr:iend,j,k,idiag) = max(c0,-T(istr:iend,j,k,nnew,iDOC))/dt
  endif
  if (wrt_bec2_diag_3d(zooczero_idx_t)) then
    idiag = idx_bec2_diag_3d(zooczero_idx_t)
    bec2_diag_3d(istr:iend,j,k,idiag) = max(c0,-T(istr:iend,j,k,nnew,iZOOC))/dt
  endif
  if (wrt_bec2_diag_3d(spcaco3zero_idx_t)) then
    idiag = idx_bec2_diag_3d(spcaco3zero_idx_t)
    bec2_diag_3d(istr:iend,j,k,idiag) = max(c0,-T(istr:iend,j,k,nnew,iSPCACO3))/dt
  endif
#endif
...
#endif
    PAR_in(i) = PAR_out(i)
    KPARdz(i) = max(spChl_loc(i)+diatChl_loc(i)+diazChl_loc(i), 0.02_8)
    ! in the following computation of KPARdz, it was taken into account that Hz is in m
    if (KPARdz(i) < 0.13224_8) then
      KPARdz(i) = 0.0919_8*(KPARdz(i)**0.3536_8) * Hz(i,j,k)
    else
      KPARdz(i) = 0.1131_8*(KPARdz(i)**0.4562_8) * Hz(i,j,k)
    end if
#if defined DAILYPAR_PHOTOINHIBITION
...
# endif
  ...
  Tfunc_diat  = 1.55_8**(((temp + T0_Kelvin) - (Tref + T0_Kelvin)) / c10)
...
# endif /* BEC2_DIAG */
    ...
    !-----------------------------------------------------------------------
# ifdef Ncycle_SY
    epsN = 1.0D-30 ! small value to avoid dividing by 0
...
# ifdef Ncycle_SY
    if (Nfixer(auto_ind)) then
      ...
      Nfix(istr:iend,auto_ind)     = (WORK1 * r_Nfix_photo) - NO3_V(istr:iend,auto_ind) - NO2_V(istr:iend,auto_in&
      &- NH4_V(istr:iend,auto_ind)
      Nexcrete(istr:iend,auto_ind) = Nfix(istr:iend,auto_ind) + NO3_V(istr:iend,auto_ind) + NO2_V(istr:iend,auto_&
...
# endif
    ...
    !  min.%C routed to POC from grazing for ballast requirements = 0.4_8 * Qcaco3
    !  min.%C routed from sp_loss = 0.59_8 * QCaCO3, or P_CaCO3%rho
    ...
    if (imp_calcifier(auto_ind)) then
      ...
      &min(spc_poc_fac * max(1.0_8,Pprime(istr:iend,auto_ind)),&
      &f_graze_sp_poc_lim))
    else
...
# endif /* BEC2_DIAG */
  ...
  zoo_loss = (parm_z_mort2_0 * Zprime**1.5_8 + parm_z_mort_0 * Zprime) * Tfunc
...
# endif
  ...
  elsewhere
    DONr_remin = DONr_loc * (c1/(365.0_8*670.0_8)) * dps ! 1/670 yrs
    DOPr_remin(istr:iend,j) = DOPr_loc * (c1/(365.0_8*460.0_8)) * dps ! 1/460 yrs
    DOC_remin = DOC_remin * 0.0685_8
    DON_remin = DON_remin * 0.1_8
    DOFe_remin = DOFe_remin * 0.05_8
    DOP_remin(istr:iend,j) = DOP_remin(istr:iend,j) * 0.05_8
  end where
# ifdef BEC2_DIAG
...
# endif
  ...
  !-----------------------------------------------------------------------
  !  Compute iron scavenging (Moore et al. 2008):
  !  1) compute in terms of loss per year per unit iron (%/year/fe)
  !  2) scale by sinking POMx10 + Dust + bSi + CaCO3 flux
  !                      POMx10 = 120 ;
  !  3) increase scavenging at higher iron (>0.6nM) (we are currently using 0.8)
  ...
  &((POC_sflux_out(istr:iend,j) + POC_hflux_out(istr:iend,j)) * 120.1_8 +&
...
# ifdef Ncycle_SY
  ...
  where (O2_loc .gt. 1e-5)
    n2o_nh4_yield1 = c1/(2.0_8 + 2.0_8 / ((parm_n2o_ji_a / O2_loc + parm_n2o_ji_b)/100.0_8));
    no2_nh4_yield1 = c1/(1.0_8 * ((parm_n2o_ji_a / O2_loc +parm_n2o_ji_b)/100.0_8) + 1.0_8);
  elsewhere
    n2o_nh4_yield1 = 0.5_8
    no2_nh4_yield1 = c0
  end where
...
! DENITRIF3 N2O --> N2
  where (N2O_loc + (ammox * n2o_nh4_yield1 + 0.5_8 * DENITRIF2&
  &- rden3 * Rden * denitrif_N2O_C) * dt  .ge. c0)
    DENITRIF3 = rden3 * Rden * denitrif_N2O_C
  elsewhere (N2O_loc .le. c0)
    DENITRIF3 = c0
  elsewhere
    DENITRIF3 = N2O_loc / dt + ammox * n2o_nh4_yield1 + 0.5_8 * DENITRIF2
  end where
...
# endif
  ...
  !-----------------------------------------------------------------------
# ifdef Ncycle_SY
  DTRACER_MODULE(istr:iend,j,k,bgc_idx(iNO3)) = nitrox - DENITRIF1 -&
...
# endif
  ...
  DTRACER_MODULE(istr:iend,j,k,bgc_idx(iNO2)) = ammox * no2_nh4_yield1 - nitrox +&
  &DENITRIF1 - DENITRIF2 - r_no2tonh4_ax * anammox - sum(NO2_V, dim=2)

  DTRACER_MODULE(istr:iend,j,k,bgc_idx(iNH4)) = -sum(NH4_V, dim=2) - ammox - anammox + DON_remin + DONr_remin&
  &+ Q * (zoo_loss_dic + sum(auto_loss_dic, dim=2) + sum(auto_graze_dic, dim=2)&
  &+ POC_remin(istr:iend,j)&
  &* (c1 - DONrefract))

  DTRACER_MODULE(istr:iend,j,k,bgc_idx(iN2O)) = ammox * n2o_nh4_yield1 + 0.5_8 * DENITRIF2 - DENITRIF3

  DTRACER_MODULE(istr:iend,j,k,bgc_idx(iN2)) = DENITRIF3 + 0.5_8 * SED_DENITRIF + anammox

# else
  DTRACER_MODULE(istr:iend,j,k,bgc_idx(iNO3)) = NITRIF - DENITRIF -&
# ifdef NO3_RESTORE
  &+ restore_no3 &
# endif
  &SED_DENITRIF - sum(NO3_V, dim=2)

  DTRACER_MODULE(istr:iend,j,k,bgc_idx(iNH4)) = -sum(NH4_V, dim=2) - NITRIF + DON_remin + DONr_remin&
  &+ Q * (zoo_loss_dic + sum(auto_loss_dic, dim=2) + sum(auto_graze_dic, dim=2)&
  &+ POC_remin(istr:iend,j)&
  &* (c1 - DONrefract))
# endif
  ...
  do auto_ind = 1, autotroph_cnt
    if (Nfixer(auto_ind))&
    &DTRACER_MODULE(istr:iend,j,k,bgc_idx(iNH4)) = DTRACER_MODULE(istr:iend,j,k,bgc_idx(iNH4))&
  ...
  DTRACER_MODULE(istr:iend,j,k,bgc_idx(iFE)) = P_iron_remin(istr:iend,j)&
  &+ (Qfe_zoo * zoo_loss_dic) + DOFe_remin - sum(photoFe, dim=2) - Fe_scavenge

  do auto_ind = 1, autotroph_cnt
    DTRACER_MODULE(istr:iend,j,k,bgc_idx(iFE)) = DTRACER_MODULE(istr:iend,j,k,bgc_idx(iFE))&
  ...
  DTRACER_MODULE(istr:iend,j,k,bgc_idx(iSIO3)) = P_SiO2_remin(istr:iend,j)

  do auto_ind = 1, autotroph_cnt
    if (kSiO3(auto_ind) > 0) then
      DTRACER_MODULE(istr:iend,j,k,bgc_idx(iSIO3)) = DTRACER_MODULE(istr:iend,j,k,bgc_idx(iSIO3))&
  ...
  DTRACER_MODULE(istr:iend,j,k,bgc_idx(iPO4)) = DOP_remin(istr:iend,j) + DOPr_remin(istr:iend,j)&
  &- sum(PO4_V, dim=2)&
  ...
  do auto_ind = 1, autotroph_cnt
    if (Qp(auto_ind) == Qp_zoo_pom) then
      DTRACER_MODULE(istr:iend,j,k,bgc_idx(iPO4)) = DTRACER_MODULE(istr:iend,j,k,bgc_idx(iPO4))&
  ...
  DTRACER_MODULE(istr:iend,j,k,bgc_idx(iZOOC)) = sum(auto_graze_zoo, dim=2) - zoo_loss
  ...
  DTRACER_MODULE(istr:iend,j,k,bgc_idx(iDOC)) = DOC_prod - DOC_remin

  DTRACER_MODULE(istr:iend,j,k,bgc_idx(iDON)) = (DON_prod * (c1 - DONrefract)) - DON_remin

  DTRACER_MODULE(istr:iend,j,k,bgc_idx(iDONR)) = (DON_prod * DONrefract) - DONr_remin&
  &+ ( POC_remin(istr:iend,j)&
  &* DONrefract * Q)

  DTRACER_MODULE(istr:iend,j,k,bgc_idx(iDOP)) = (DOP_prod * (c1 - DOPrefract)) - DOP_remin(istr:iend,j)&
  &- sum(DOP_V, dim=2)

  DTRACER_MODULE(istr:iend,j,k,bgc_idx(iDOPR)) = (DOP_prod * DOPrefract) - DOPr_remin(istr:iend,j)&
  &+ ( POC_remin(istr:iend,j)&
  &* DOPrefract * Qp_zoo_pom)

  DTRACER_MODULE(istr:iend,j,k,bgc_idx(iDOFE)) = DOFe_prod - DOFe_remin

  !-----------------------------------------------------------------------
  !   dissolved inorganic Carbon
  !-----------------------------------------------------------------------

  DTRACER_MODULE(istr:iend,j,k,bgc_idx(iDIC)) =&
  ...
  do auto_ind = 1, autotroph_cnt
    ...
    &DTRACER_MODULE(istr:iend,j,k,bgc_idx(iDIC)) = DTRACER_MODULE(istr:iend,j,k,bgc_idx(iDIC))&
  ...
  DTRACER_MODULE(istr:iend,j,k,bgc_idx(iALK)) = -DTRACER_MODULE(istr:iend,j,k,bgc_idx(iNO3)) +&
  &DTRACER_MODULE(istr:iend,j,k,bgc_idx(iNH4)) + c2&
  &* P_CaCO3_remin(istr:iend,j)

  do auto_ind = 1, autotroph_cnt
    if (CaCO3_ind(auto_ind) > 0)&
    &DTRACER_MODULE(istr:iend,j,k,bgc_idx(iALK)) = DTRACER_MODULE(istr:iend,j,k,bgc_idx(iALK))&
  ...
  do auto_ind = 1, autotroph_cnt
    if (.not. Nfixer(auto_ind)) then
      ...
      where (photoC(istr:iend,auto_ind) > c0)
        O2_PRODUCTION = O2_PRODUCTION + photoC(istr:iend,auto_ind) *&
        &((NO3_V(istr:iend,auto_ind) / (NO3_V(istr:iend,auto_ind)&
        &+ NO2_V(istr:iend,auto_ind) + NH4_V(istr:iend,auto_ind) + Nfix(istr:iend,auto_ind))) / parm_Red_D_C_O&
...
# endif
  DTRACER_MODULE(istr:iend,j,k,bgc_idx(iO2)) = O2_PRODUCTION - O2_CONSUMPTION
...
&iend,j,net_dust_in)
  ...
  use bec2_params, only:&
  &c0, c1, parm_caco3_diss, parm_poc_diss, parm_sio2_diss
  use bec2_vars, only:&
  &dust_sflux_out, dust_hflux_out, dust_diss,&
  &dust_gamma, dust_mass, dust_rho, p_caco3_diss, p_caco3_gamma,&
  &p_caco3_hflux_out, p_caco3_mass, p_caco3_rho, p_caco3_sflux_out,&
  &p_iron_gamma, p_iron_hflux_out, p_iron_sflux_out, p_sio2_diss,&
  &p_sio2_gamma, p_sio2_hflux_out, p_sio2_mass, p_sio2_rho,&
  &p_sio2_sflux_out, poc_diss, poc_gamma, poc_hflux_out, poc_mass,&
  &poc_sflux_out
  ...
  POC_mass      = 12.01_8           ! molecular weight of POC
  POC_gamma     = c0

  P_CaCO3_diss  = parm_CaCO3_diss ! diss. length (m)
  P_CaCO3_gamma = 0.30_8            ! prod frac -> hard subclass
  P_CaCO3_mass  = 100.09_8          ! molecular weight of CaCO3
  P_CaCO3_rho   = 0.05_8 * P_CaCO3_mass / POC_mass ! QA mass ratio for CaCO3

  P_SiO2_diss   = parm_SiO2_diss  ! diss. length (m), modified by TEMP
  P_SiO2_gamma  = 0.030_8           ! prod frac -> hard subclass
  P_SiO2_mass   = 60.08_8           ! molecular weight of SiO2
  P_SiO2_rho    = 0.05_8 * P_SiO2_mass / POC_mass ! QA mass ratio for SiO2

  dust_diss     = 200.0_8           ! diss. length (m) (DL: changed from cm)
  dust_gamma    = 0.97_8            ! prod frac -> hard subclass
  dust_mass     = 1.0D6           ! base units are already kg
  dust_rho      = 0.05_8 * dust_mass / POC_mass ! QA mass ratio for dust
  ...
  !      Note : dust_rho converts dust_flux from kg/m2/s to mmolC/m2/s
  !-----------------------------------------------------------------------
...
&OTHER_REMIN,istr,iend,j)
  ...
  use ocean_vars, only: z_w, hz
  use bec2_params, only:&
  &c0, c1, c10, denitrif_c_n, dust_to_fe,&
  &parm_bsibury, parm_lowo2_remin_factor, parm_pombury,&
  &parm_red_fe_c, parm_sed_denitrif_no3_limit, q_10, spd, tref,&
  &parm_scalelen_z, parm_scalelen_vals
  use bec2_vars, only:&
#ifdef BEC2_DIAG
  bec2_diag_3d, caco3fluxin_idx_t,&
  &caco3fluxtosed_idx_t, caco3remin_idx_t, caco3sedloss_idx_t,&
  &dustfluxin_idx_t, dustfluxtosed_idx_t, dustremin_idx_t,&
  &fesedflux_idx_t, fluxtosed_idx_t, otherremin_idx_t,&
  &pironfluxin_idx_t, pironfluxtosed_idx_t, pironremin_idx_t,&
  &pocfluxin_idx_t, pocremin_idx_t, pocsedloss_idx_t,&
  &seddenitrif_idx_t, sio2fluxin_idx_t, sio2fluxtosed_idx_t,&
  &sio2remin_idx_t, sio2sedloss_idx_t, wrt_bec2_diag_3d,&
  &idx_bec2_diag_3d, wrt_bec2_diag_2d, idx_bec2_diag_2d,&
  &bec2_diag_2d, &
#endif
  &p_caco3_sflux_in, p_caco3_sflux_out,&
  &p_caco3_hflux_in, p_caco3_hflux_out, p_sio2_sflux_in,&
  &p_sio2_sflux_out, p_sio2_hflux_in, p_sio2_hflux_out,&
  &dust_sflux_in, dust_sflux_out, dust_hflux_in, dust_hflux_out,&
  &poc_sflux_in, poc_sflux_out, poc_hflux_in, poc_hflux_out,&
  &p_iron_sflux_in, p_iron_sflux_out, p_iron_hflux_in,&
  &p_iron_hflux_out, p_iron_sed_loss, poc_sed_loss,&
  &p_caco3_sed_loss, p_sio2_sed_loss, dust_sed_loss, dust_diss,&
  &dust_rho, p_caco3_diss, p_caco3_gamma, p_caco3_rho,&
  &p_iron_gamma, p_sio2_diss, p_sio2_gamma, p_sio2_rho, poc_diss,&
  &landmask, p_caco3_remin, p_sio2_remin, poc_remin, p_iron_remin

  implicit none

  integer(kind=4) istr, iend, i, j, k, n_loc, iB
  integer(kind=4) idiag
  real(kind=8), dimension(istr:iend) ::&
  &temp, O2_loc, NO3_loc&
  ...
  logical poc_error      ! POC error flag
...
# endif
  ...
  do i = istr, iend
    ...
    DECAY_Hard(i)     = exp(-Hz(i,j,k) / 4.0D4)  ! DL: denominator converted to m
    DECAY_HardDust(i) = exp(-Hz(i,j,k) / 1.2D5)  ! DL: denominator converted to m
  ...
  do i = istr, iend
    ...
    if (LANDMASK(i,j)) then
      ...
      if ((O2_loc(i) >= 5.0_8) .and. (O2_loc(i) < 40.0_8)) then
        poc_diss_loc = POC_diss*(c1+(parm_lowo2_remin_factor-c1)*(40.0_8 - O2_loc(i))/35.0_8)
      else if (O2_loc(i) < 5.0_8) then
...
# endif
      ...
      !
      !  dust remin gDust = 0.035_8 / 55.847_8 * 1.0D9 = 626712.0 nmolFe
      !                      gFe     molFe     nmolFe
...
!        endif
    ...
    !  Using empirical relations from Bohlen et al., 2012 (doi:10.1029_8/2011GB004198) for Sed Denitrification
    ...
    !  POC burial from Dunne et al. 2007 (doi:10.1029_8/2006GB002907), maximum of 80% burial efficiency imposed
    !  Bsi preservation in sediments based on
    !     Ragueneau et al. 2000 (doi:10.1016_8/S0921-8181(00)00052-7)
    ...
    if (LANDMASK(i,j) .and. (k == 1)) then
      ...
      if (flux > c0) then
        ...
        POC_sed_loss(i,j) = flux * min(0.8_8, parm_POMbury&
        &* (0.013_8 + 0.53_8 * flux_alt*flux_alt / (7.0_8 + flux_alt)**2))
        ...
        if (NO3_loc(i) < parm_sed_denitrif_NO3_limit) then
          SED_DENITRIF(i) = dzr_loc * min(flux * (0.06_8 + 0.19_8 *&
          &0.99_8**(O2_loc(i)-NO3_loc(i))),&
        ...
        flux_alt = flux*1e-4*spd*365.0_8 ! convert to mmol C/cm^2/year
        OTHER_REMIN(i) = dzr_loc&
        &* min(min(0.1_8 + flux_alt,0.5_8) * (flux - POC_sed_loss(i,j)),&
        ...
        if (flux_oxidated(i) > 0) then
          FESEDFLUX(i) = 170.0_8 * tanh(flux_oxidated(i)/O2_loc(i))
        endif
        ! convert from umol m-2 d-1 to mmol m-2 s-1
!            if (latr(i,j) > 5.0_8 .and. latr(i,j) < 20.0_8) then
!                FESEDFLUX(i) = FESEDFLUX(i)*0.001_8/86400*1.0_8     !sec2day=1/86400
!            else
        FESEDFLUX(i) = FESEDFLUX(i)*0.001_8/86400*1.0_8
!            endif
# ifdef BEC2_DIAG
...
# endif
      ...
      if (flux_alt > c1) then
        P_SiO2_sed_loss(i,j) = 0.3_8 * flux_alt - 0.06_8
      else
        P_SiO2_sed_loss(i,j) = 0.04_8
      endif
...
! needed for rho0
  ...
  REAL(kind=8), DIMENSION(istr:iend,jstr:jend), INTENT(OUT)  :: WORK ! 10m windspeed (m/s)
  ...
  !-----------------------------------------------------------------------
  real(kind=8),parameter::&
  &rho_air   = 1.2_8            ! ambient air density (kg/m^3)

  !-----------------------------------------------------------------------
  !   arguments
  !-------------------------------------------------------------------------

  REAL(kind=8), DIMENSION(istr:iend,jstr:jend), INTENT(IN) ::&
  ...
  REAL(kind=8), PARAMETER ::&
  &coef_1  = 0.0027_8,&     ! (m/s)
  &coef_2  = 0.000142_8,&   ! (non-dimensional)
  &coef_3  = 0.0000764_8,&  ! (s/m)
  &c_d     = 1.7D-3      ! drag coefficient for initial u
...
&iend,jstr,jend)
  ...
  REAL(kind=8), DIMENSION(istr:iend,jstr:jend), INTENT(IN) :: SSTT

  !---------------------------------------------------------------------------
  !   result declaration
  !---------------------------------------------------------------------------

  REAL(kind=8), DIMENSION(istr:iend,jstr:jend),intent(out) :: SCHMIDT_N2O
...
&iend,jstr,jend)
  ...
  use bec2_params, only: c0, t0_kelvin
...
!#include "bgc_param_bec2.h"
  ...
  REAL(kind=8), DIMENSION(istr:iend,jstr:jend),intent(out) :: N2OSAT

  !---------------------------------------------------------------------------
  !   local variables
  !---------------------------------------------------------------------------

  REAL(kind=8), DIMENSION(istr:iend,jstr:jend) :: TS, S_LOC
  ...
  S_LOC = max(1.00D-4,SSSS)

  WHERE (LANDMASKC)
    TS = T0_Kelvin + SSTT

    N2OSAT =  exp(a_1 + a_2 * (100.0_8/TS) + a_3 * log(TS/100.0_8) +&
    &a_4 * (TS/100.0_8)**2 + S_LOC * (b_1 +b_2 * (TS/100.0_8) + b_3 *&
    &(TS/100.0_8)**2))
  ELSEWHERE
    N2OSAT = c0
  END WHERE
...
&iend,jstr,jend)
  ...
  !   where LAND_MASK is true. Give zero where LAND_MASK is false.
  !
  !   ref : Sarmiento and Gruber 2006 (Table 3.3.1_8)
  !
  !---------------------------------------------------------------------------
  !
  ! over land, do NOT assign zero to the result, but use very large value instead
  ...
  REAL(kind=8), DIMENSION(istr:iend,jstr:jend),intent(out) :: SCHMIDT_N2
...
&iend,jstr,jend)
  ...
  use bec2_params, only: c0, c1, t0_kelvin
  ...
  !   --> Solubility : table 3.2.2_8
  !   --> apply correction for moist air following Panel 3.2.1_8
  !
  ...
  REAL(kind=8), DIMENSION(istr:iend,jstr:jend),intent(out) :: N2SAT

  !---------------------------------------------------------------------------
  !   local variables
  !---------------------------------------------------------------------------

  REAL(kind=8), DIMENSION(istr:iend,jstr:jend) :: TS, BUNSEN, PH2O_to_P,&
  &S_LOC
  ...
  &b_2 = 0.026329_8,&
  &b_3 = -0.0037252_8,&
  &vbarn2o = 22.4136_8 ! volar molume of N2 at standard temperature and pressure
  ...
  WHERE (LANDMASKC)
    ...
    BUNSEN=  exp(a_1 + a_2 * (100.0_8/TS) + a_3 * log(TS/100.0_8) +&
    &S_LOC * (b_1 +b_2 * (TS/100.0_8) + b_3 *&
    &(TS/100.0_8)**2))

    PH2O_to_P = exp(24.4543_8-67.4509_8*(100.0_8/TS)-4.8489_8*log(TS/100.0_8)&
    &-0.000544_8*S_LOC)
...
&iend,jstr,jend)
  ...
  REAL(kind=8), DIMENSION(istr:iend,jstr:jend),intent(out) :: SCHMIDT_O2
...
&iend,jstr,jend)
  ...
  REAL(kind=8), DIMENSION(istr:iend,jstr:jend),intent(out) :: O2SAT
...
&iend,jstr,jend )
  ...
  REAL(kind=8), DIMENSION(istr:iend,jstr:jend), INTENT(OUT) :: SCHMIDT_CO2
...
&dco2star, pco2oc, dpco2, istr,iend)
  ...
  DO i = istr,iend
    IF (mask(i)) THEN
      ...
      is       = 19.924_8 * s_loc(i) / (c1000 - 1.005_8 * s_loc(i))
      is2      = is * is
      sqrtis   = SQRT(is)
      sqrts    = SQRT(s_loc(i))
      s15      = s_loc(i) ** 1.5_8
      s2       = s_loc(i) ** 2
      scl      = s_loc(i) / 1.80655_8
      ...
      &90.9241_8*LOG(tk100) -&
      &1.47696_8*tk1002 + s_loc(i)*(.025695_8 - .025225_8*tk100 +&
      &0.0049867_8*tk1002))
      ...
      k0(i) = EXP(93.4517_8/tk100 - 60.2409_8 + 23.3585_8*LOG(tk100) +&
      &s_loc(i)*(0.023517_8 - 0.023656_8 * tk100 +&
      &0.0047036_8 * tk1002))
      ...
      &0.0118_8*s_loc(i) + 0.000116_8*s2))

      k2(i) = 10**(-1*(1394.7_8*invtk + 4.777_8 -&
      &0.0184_8*s_loc(i) + 0.000118_8*s2))
      ...
      kb(i) = EXP((-8966.90_8 - 2890.53_8*sqrts - 77.942_8*s_loc(i) +&
      &1.728_8*s15 - 0.0996_8*s2)*invtk +&
      &(148.0248_8 + 137.1942_8*sqrts + 1.62142_8*s_loc(i)) +&
      &(-24.4344_8 - 25.085_8*sqrts - 0.2474_8*s_loc(i)) *&
      ...
      k1p(i) = EXP(-4576.752_8*invtk + 115.525_8 - 18.453_8 * dlogtk +&
      &(-106.736_8*invtk + 0.69171_8) * sqrts +&
      &(-0.65643_8*invtk - 0.01844_8) * s_loc(i))
      ...
      k2p(i) = EXP(-8814.715_8*invtk + 172.0883_8 - 27.927_8 * dlogtk +&
      &(-160.340_8*invtk + 1.3566_8) * sqrts +&
      &(0.37335_8*invtk - 0.05778_8) * s_loc(i))
      ...
      &(-44.99486_8*invtk - 0.09984_8) * s_loc(i))
      ...
      kw(i) = EXP(-13847.26_8*invtk + 148.9652_8 - 23.6521_8 * dlogtk +&
      &(118.67_8*invtk - 5.977_8 + 1.0495_8 * dlogtk) *&
      &sqrts - 0.01615_8 * s_loc(i))
  ...
  !   set x1 and x2 to the previous value of the pH +/- ~0.5_8._8
  !---------------------------------------------------------------------------
...
end subroutine ecosys_bec2_tile_empty
#endif  /* BIOLOGY_BEC2 */
