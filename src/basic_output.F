...
#include "cppdefs.opt"
     ...
     &     w_avg, wvl_avg, zeta, ubar, vbar, u, v, z_r, We, Wi
      ...
      real, public :: output_period_rst = 100        ! output period in seconds
      real    :: nrpf_rst          = 10

      real    :: output_period_his = 100         ! output period in seconds
      integer :: nrpf_his          = 20          ! total recs per file

      real    :: output_period_avg = 100         ! output averaging period in seconds
      integer :: nrpf_avg          = 20          ! total recs per file
...
! ----------------------------------------------------------------------
     ...
     &     , hbbl
      ...
      integer                :: total_rec_his=0                      ! total his output records so far
...
# ifdef SOLVE3D
        ...
        if (wrt_avg_O) then
          ...
          enddo
        pio_gtype = '3Dww'
          call ncwrite(ncid, vname(1,indxO),     w_avg(i0:i1,j0:j1,:), start,.true.)            ! here rather than calc_avg
        endif
...
!-----------------------------------------------------------------------
     ...
     &        "Name","Write (T/F)","Long name"
         write(*,'(9x,A)')  repeat('-',62)
         write(*,'(13(/11x,A,T30,L1,T36,A))') ! 13(....) repeats formatting 11 times.
