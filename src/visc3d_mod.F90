...
contains
  ...
  subroutine visc3d_s
    ...
    implicit none
    integer(kind=4),parameter :: tile=0

# include "compute_tile_bounds.h"
    call visc3d_s_tile (istr,iend,jstr,jend, A2d(1,1), A2d(1,2),&
    &A2d(1,3), A2d(1,4))
  end subroutine visc3d_s

  subroutine visc3d_s_tile (istr,iend,jstr,jend, UFx,UFe,VFx,VFe)
...
!            the large-scale ocean circulation, JCP, 105, 333-338._8

    use coupling, only: rufrc, rvfrc
    use grid, only: pm, pn, dm_r, dn_r, pmask, dm_p, dn_p, nx,ny
...
# ifdef DIAGNOSTICS
      if (diag_uv .and. calc_diag) then
        do j=1,ny
          do i=1,nx
            Udiag(i,j,k,ihmix) = 0.5_8*dxdyi_u(i,j)*&
            &( (pn(i-1,j)+pn(i,j))*(UFx(i,j)-UFx(i-1,j))&
            &+(pm(i-1,j)+pm(i,j))*(UFe(i,j+1)-UFe(i,j)) )
            Vdiag(i,j,k,ihmix) = 0.5_8*dxdyi_v(i,j)*&
            &( (pn(i,j-1)+pn(i,j))*(VFx(i+1,j)-VFx(i,j))&
            &+(pm(i,j-1)+pm(i,j))*(VFe(i,j)-VFe(i,j-1)) )
          enddo
        enddo
...
# endif
  ...
  end subroutine visc3d_s_tile

  subroutine visc3d_GP
...
#include "compute_tile_bounds.h"
    ...
    call visc3d_GP_tile (istr,iend,jstr,jend,&
  ...
  end subroutine visc3d_GP

  subroutine visc3d_GP_tile (istr,iend,jstr,jend, UFx, UFe,&
...
!       No. 8, pp. 2935-2946._8
    ...
    use mixing, only: visc2_r, visc2_p
    use ocean_vars, only: z_r, u, v, hz
    use scalars, only: dt, nstp
    use dimensions, only: inode, jnode

    implicit none
    integer(kind=4) istr,iend,jstr,jend, i,j,k, k1,k2, indx
    real(kind=8), dimension(PRIVATE_2D_SCRATCH_ARRAY) :: UFx,UFe,VFe,VFx
    real(kind=8), dimension(PRIVATE_2D_SCRATCH_ARRAY,2) :: UFs,VFs,&
    &dmUde,dmVde,dnUdx,dnVdx, dUdz,dVdz,&
    &dZde_p,dZde_r,dZdx_p,dZdx_r

    real(kind=8) cff, cff1,cff2,cff3,cff4, cff5,cff6,cff7,cff8,&
...
# endif
        ...
        do j=jstrV-1,jend+1
          do i=istr-1,iend+1
            VFe(i,j)=0.5_8*(z_r(i,j,k+1)-z_r(i,j-1,k+1))&
            &*(pn(i,j-1)+pn(i,j))&
# ifdef MASKING
            &*vmask(i,j)
# endif
        ...
        do j=jstrV-1,jend
          do i=istrU-1,iend
            dnUdx(i,j,k2)=0.5_8*pm(i,j)*(&
            &(pn(i  ,j)+pn(i+1,j))*u(i+1,j,k+1,nstp)&
            &-(pn(i-1,j)+pn(i  ,j))*u(i  ,j,k+1,nstp))&
# ifdef MASKING
            &*rmask(i,j)
# endif
            dmVde(i,j,k2)=0.5_8*pn(i,j)*(&
            &(pm(i,j  )+pm(i,j+1))*v(i,j+1,k+1,nstp)&
            &-(pm(i,j-1)+pm(i,j  ))*v(i,j  ,k+1,nstp))&
# ifdef MASKING
            &*rmask(i,j)
# endif
            dZdx_r(i,j,k2)=0.5_8*(UFx(i,j)+UFx(i+1,j))
            dZde_r(i,j,k2)=0.5_8*(VFe(i,j)+VFe(i,j+1))
          enddo
        enddo
        do j=jstr,jend+1
          do i=istr,iend+1
            dmUde(i,j,k2)=0.125_8*(pn(i,j)+pn(i-1,j)+pn(i,j-1)&
            &+pn(i-1,j-1))&
...
# ifdef MASKING
            &*pmask(i,j)
# endif
            dnVdx(i,j,k2)=0.125_8*(pm(i,j)+pm(i-1,j)+pm(i,j-1)&
            &+pm(i-1,j-1))&
...
# ifdef MASKING
            &*pmask(i,j)
# endif
            dZde_p(i,j,k2)=0.5_8*(VFe(i-1,j)+VFe(i,j))
            dZdx_p(i,j,k2)=0.5_8*(UFx(i,j-1)+UFx(i,j))
          enddo
        enddo          !--> discard UFx,VFe, keep all others
      endif


!        do j=jstrV-1,jend+1       ! This
!          do i=istrU-1,iend+1
!            dZdx_r(i,j,k2)=0._8
!            dZde_r(i,j,k2)=0._8
!            dZdx_p(i,j,k2)=0._8
...
!        enddo
      ...
      if (k==0 .or. k==N) then
        ...
        do j=jstr-1,jend+1
          do i=istrU-1,iend+1
            dUdz(i,j,k2)=2._8*(u(i,j,k+1,nstp)-u(i,j,k,nstp))&
        ...
        do j=jstrV-1,jend+1
          do i=istr-1,iend+1
            dVdz(i,j,k2)=2._8*(v(i,j,k+1,nstp)-v(i,j,k,nstp))&
      ...
      if (k>0) then
        do j=jstrV-1,jend   ! UFx at rho-points hence this covers interior (non-boundary)
          do i=istrU-1,iend
            ...
            &dn_r(i,j)*( dnUdx(i,j,k1) - 0.5_8*pn(i,j)*(&
            &min(dZdx_r(i,j,k1),0._8)*(dUdz(i,j,k1)+dUdz(i+1,j,k2))&
            &+max(dZdx_r(i,j,k1),0._8)*(dUdz(i,j,k2)+dUdz(i+1,j,k1))&
            &))&
            &-dm_r(i,j)*( dmVde(i,j,k1) - 0.5_8*pm(i,j)*(&
            &min(dZde_r(i,j,k1),0._8)*(dVdz(i,j,k1)+dVdz(i,j+1,k2))&
            &+max(dZde_r(i,j,k1),0._8)*(dVdz(i,j,k2)+dVdz(i,j+1,k1))&
            &)))&
# ifdef MASKING
            &*rmask(i,j)
# endif
        ...
        do j=jstr,jend+1
          do i=istr,iend+1
            cff=visc2_p(i,j)*0.25_8*( Hz(i,j,k) +Hz(i-1,j,k)&
            &+Hz(i,j-1,k) +Hz(i-1,j-1,k))&
            &*( dn_p(i,j)*( dnVdx(i,j,k1)-0.125_8*( pn(i,j)+pn(i-1,j)&
            &+pn(i,j-1)+pn(i-1,j-1))&
            &*( min(dZdx_p(i,j,k1),0._8)*(dVdz(i-1,j,k1)+dVdz(i,j,k2))&
            &+max(dZdx_p(i,j,k1),0._8)*(dVdz(i-1,j,k2)+dVdz(i,j,k1))&

            &)) + dm_p(i,j)*( dmUde(i,j,k1)-0.125_8*( pm(i,j)+pm(i-1,j)&
            &+pm(i,j-1)+pm(i-1,j-1))&
            &*( min(dZde_p(i,j,k1),0._8)*(dUdz(i,j-1,k1)+dUdz(i,j,k2))&
            &+max(dZde_p(i,j,k1),0._8)*(dUdz(i,j-1,k2)+dUdz(i,j,k1))&
            &)))&
# ifdef MASKING
            &*pmask(i,j)
# endif
        ...
        if (k<N) then
          do j=jstr,jend
            do i=istrU,iend
              ...
              cff2=0.5_8*(pm(i-1,j)+pm(i,j))
              cff=0.25_8*( dVdz(i,j,k2)+dVdz(i-1,j,k2)&
              ...
              cff1=min(dZdx_r(i-1,j,k1),0._8)
              cff2=min(dZdx_r(i  ,j,k2),0._8)
              cff3=max(dZdx_r(i-1,j,k2),0._8)
              cff4=max(dZdx_r(i  ,j,k1),0._8)
              cff5=min(dZde_r(i-1,j,k1),0._8)
              cff6=min(dZde_r(i  ,j,k2),0._8)
              cff7=max(dZde_r(i-1,j,k2),0._8)
              cff8=max(dZde_r(i  ,j,k1),0._8)
              ...
              &)
              cff1=min(dZde_p(i,j  ,k1),0._8)
              cff2=min(dZde_p(i,j+1,k2),0._8)
              cff3=max(dZde_p(i,j  ,k2),0._8)
              cff4=max(dZde_p(i,j+1,k1),0._8)
              cff5=min(dZdx_p(i,j  ,k1),0._8)
              cff6=min(dZdx_p(i,j+1,k2),0._8)
              cff7=max(dZdx_p(i,j  ,k2),0._8)
              cff8=max(dZdx_p(i,j+1,k1),0._8)
              ...
              UFs(i,j,k2)=0.25_8*(visc2_r(i-1,j)+visc2_r(i,j))*cff
            enddo
          enddo

          do j=jstrV,jend
            do i=istr,iend
              cff1=0.5_8*(pn(i,j-1)+pn(i,j))
              cff2=0.5_8*(pm(i,j-1)+pm(i,j))
              cff=0.25_8*( dUdz(i,j,k2)+dUdz(i+1,j,k2)&
              ...
              cff1=min(dZdx_p(i  ,j,k1),0._8)
              cff2=min(dZdx_p(i+1,j,k2),0._8)
              cff3=max(dZdx_p(i  ,j,k2),0._8)
              cff4=max(dZdx_p(i+1,j,k1),0._8)
              cff5=min(dZde_p(i  ,j,k1),0._8)
              cff6=min(dZde_p(i+1,j,k2),0._8)
              cff7=max(dZde_p(i  ,j,k2),0._8)
              cff8=max(dZde_p(i+1,j,k1),0._8)
              ...
              &)
              cff1=min(dZde_r(i,j-1,k1),0._8)
              cff2=min(dZde_r(i,j  ,k2),0._8)
              cff3=max(dZde_r(i,j-1,k2),0._8)
              cff4=max(dZde_r(i,j  ,k1),0._8)
              cff5=min(dZdx_r(i,j-1,k1),0._8)
              cff6=min(dZdx_r(i,j  ,k2),0._8)
              cff7=max(dZdx_r(i,j-1,k2),0._8)
              cff8=max(dZdx_r(i,j  ,k1),0._8)
              ...
              VFs(i,j,k2)=0.25_8*(visc2_r(i,j-1)+visc2_r(i,j))*cff
            enddo
          enddo
        endif
...
# endif
  ...
  end subroutine visc3d_GP_tile

#else
contains
  subroutine visc3d_empty
  end subroutine visc3d_empty
#endif
end module visc3d_mod
