...
This ``ci/`` script has no existing test coverage / pytest CI wiring in this
repo; run directly with::
...
def test_plain_span_converted():
    assert _md_code_to_rst("Ability to specify `reference_date` in the namelist") == (
        "Ability to specify ``reference_date`` in the namelist"
    )
...
def test_explicit_target_ref_preserved():
    text = "fixed a bug (`#300 <https://github.com/CWorthy-ocean/ucla-roms/pull/300>`_)"
