...
https://github.com/CWorthy-ocean/ucla-roms and inserts their
...
# We use the PR-template section names verbatim, so this is an identity map.
...
def main() -> None:
    ...
    # strictly newer than the last tag.  If none exists yet (e.g. only
    # unreleased.rst, which has no parseable version), use (or create)
    # unreleased.rst.
    try:
        version_stem, candidate_path = get_latest_rst()
    except RuntimeError:
        version_stem, candidate_path = None, None
    if candidate_path is not None and _parse_version(version_stem) > _parse_version(tag_name):
