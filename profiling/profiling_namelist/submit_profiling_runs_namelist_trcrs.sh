...
# Number of Tracers. NP_TRC iterables:
NP_TRC=(32 64 100)
...
for idT in "${!NP_TRC[@]}"; do
    for idx in "${!NP_X[@]}"; do
        ...
        thisdir="${topdir}/run_${idx}_npx_${npx}_npy_${npy}_trcr_${trc}"
        cp -rpv ../template_configuration_namelist_tracers/ ${thisdir}/
        ...
        ln -s ${INPUT_DATA_DIR}/parent_phys_boundary_forcing_trcrs.nc .
        ln -s ${INPUT_DATA_DIR}/parent_bgc_boundary_forcing.nc .
        ln -s ${INPUT_DATA_DIR}/parent_rst_trcrs.20120201000000.nc .
    
        cd $thisdir/
        sed -i -e "s/TEMPLATE_NPX/${npx}/g" namelist.nml
        sed -i -e "s/TEMPLATE_NPY/${npy}/g" namelist.nml
        sed -i -e "s/TEMPLATE_ntpass/${trc}/g" namelist.nml
